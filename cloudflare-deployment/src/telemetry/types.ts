 ...
 *
 * - `memory`: in-process ring buffer only (lost on DO eviction).
 * - `kv`: Cloudflare KV-backed; unique-key writes sidestep the 1-write/sec/key limit.
 * - `filesystem`: unsupported in Workers; treated as `memory` with a warning.
 * - `external`: external API; stubbed.
 */
export type TelemetryStorage = 'memory' | 'kv' | 'filesystem' | 'external';
