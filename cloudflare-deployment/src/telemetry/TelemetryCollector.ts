...
import { onShutdown, offShutdown, type PlatformTimeout } from '../platform/lifecycle.js';

export class TelemetryCollector {
  private privacyManager: PrivacyManager;
  private storage: TelemetryStorage;
  private eventBuffer: TelemetryEvent[] = [];
  private flushTimer?: PlatformTimeout;
  private sessionStartTimes = new Map<string, number>();
  private isShuttingDown = false;
  private shutdownHandler?: () => void;

  constructor(
    private config: TelemetryConfig = PrivacyManager.getConfigFromEnvironment(),
    kvNamespace: KVNamespace | null = null
  ) {
    this.privacyManager = new PrivacyManager(config);
    this.storage = new TelemetryStorage(config, kvNamespace);
  ...
  async shutdown(): Promise<void> {
    ...
    if (this.shutdownHandler) {
      offShutdown(this.shutdownHandler);
    }
  ...
  /**
   * Replace the global instance with one bound to a specific KV namespace.
   * Called from the DO constructor so telemetry writes can reach Workers KV.
   */
  static configureInstance(
    ...
    if (TelemetryCollector.instance) {
      void TelemetryCollector.instance.shutdown();
    }
    TelemetryCollector.instance = new TelemetryCollector(config, kvNamespace);
    return TelemetryCollector.instance;
  }
}
