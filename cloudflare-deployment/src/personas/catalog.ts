...
/**
 * External persona loading is not supported in the Cloudflare Workers build.
 * Kept as a stub so code that imports it continues to compile; always returns `{}`.
 */
export function loadExternalPersonas(_path: string): Record<string, PersonaDefinition> {
  return {};
}

let externalCatalogWarned = false;
let catalogCache: Record<string, PersonaDefinition> | null = null;

/**
 * Get the catalog — built-in personas only in Workers.
 * `PERSONA_CATALOG_PATH` is ignored; a one-time warning is emitted if it is set.
 */
export function getMergedCatalog(): Record<string, PersonaDefinition> {
  ...
  if (externalPath && !externalCatalogWarned) {
    logger.warn(
      '[Personas] PERSONA_CATALOG_PATH is not supported in the Cloudflare Workers build; using built-in personas only'
    );
    externalCatalogWarned = true;
  }
...
/**
 * Invalidate the cached catalog. No-op effect on personas (built-in only) but
 * useful for tests that need to re-trigger the external-path warning branch.
 */
export function invalidateCatalogCache(): void {
  catalogCache = null;
  externalCatalogWarned = false;
}
