...
export class EscapeVelocitySystem {
  private calculator = new EscapeVelocityCalculator();
  private protocolFactory = new EscapeProtocolFactory();
  private monitoring: EscapeMonitoring = {
    attemptCount: 0,
    successCount: 0,
   ...
   */
  getAvailableProtocols(currentFlexibility: number) {
  ...
  executeProtocol(level: EscapeLevel, context: EscapeContext): EscapeAttemptResult {
    ...
    if (protocol.requiredFlexibility > context.currentFlexibility.flexibilityScore) {
      throw new Error(
        `Insufficient flexibility for ${protocol.name}. ` +
   ...
   */
  recommendProtocol(currentFlexibility: number, constraintStrength: number) {
  ...
  getEscapeUrgency(flexibility: number): 'critical' | 'high' | 'medium' | 'low' {
    if (flexibility < 0.1) return 'critical';
    if (flexibility < 0.2) return 'high';
    if (flexibility < 0.3) return 'medium';
    return 'low';
  }
   ...
   */
  private updateMonitoring(result: EscapeAttemptResult): void {
    ...
    const totalGain =
      this.monitoring.averageFlexibilityGain * (this.monitoring.attemptCount - 1) +
