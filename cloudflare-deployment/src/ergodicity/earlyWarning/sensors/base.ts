...
export abstract class Sensor {
   ...
   */
  async measure(pathMemory: PathMemory, sessionData: SessionData): Promise<SensorReading> {
    // Get raw sensor data
   ...
   */
  protected abstract getRawReading(
   ...
   * Must be implemented by each sensor
   */
  protected abstract detectIndicators(
    pathMemory: PathMemory,
    sessionData: SessionData
  ): Promise<string[]>;
  ...
  protected applyNoiseFilter(rawValue: number): number {
    ...
    if (change < this.calibration.noiseFilter) {
      return (
        this.lastReading.rawValue * this.calibration.historicalWeight +
