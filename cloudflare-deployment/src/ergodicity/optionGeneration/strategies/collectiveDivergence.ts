...
export class CollectiveDivergenceStrategy extends BaseOptionStrategy {
  readonly strategyName = 'collective_divergence' as const;
  readonly description =
    'Create flexibility by leveraging collective intelligence and encouraging productive divergence';
  readonly typicalFlexibilityGain = { min: 0.3, max: 0.5 };
  readonly applicableCategories: OptionCategory[] = ['relational', 'process', 'capability'];

  isApplicable(context: OptionGenerationContext): boolean {
    // Look for situations where individual thinking is limited
    const hasIndividualLimitations = context.pathMemory.constraints.some(
      c =>
        c.description.toLowerCase().includes('limited perspective') ||
        c.description.toLowerCase().includes('blind spot') ||
        c.description.toLowerCase().includes('expertise gap')
    );

    const hasGroupPotential = context.pathMemory.pathHistory.some(
      event =>
        event.decision.toLowerCase().includes('team') ||
        event.decision.toLowerCase().includes('collective') ||
        event.decision.toLowerCase().includes('collaborate')
    );

    const needsDiversePerspectives =
      context.currentFlexibility.flexibilityScore < 0.35 ||
  ...
  estimateEffort(option: Option): 'low' | 'medium' | 'high' {
    if (option.name.includes('Brainstorming')) return 'medium';
    if (option.name.includes('Multiplication')) return 'low';
    if (option.name.includes('Sensing') || option.name.includes('Aggregation')) return 'high';
    return 'medium';
  }

  private needsCollectiveSensing(context: OptionGenerationContext): boolean {
    // Check if there are weak signals or emerging patterns
    const hasUncertainty = context.pathMemory.constraints.some(
      c =>
        c.description.toLowerCase().includes('uncertain') ||
        c.description.toLowerCase().includes('emerging') ||
        c.description.toLowerCase().includes('unclear')
    );

    const hasComplexEnvironment =
      context.pathMemory.pathHistory.length > 15 &&
      context.pathMemory.flexibilityOverTime.some(
        (f, i, arr) => i > 0 && Math.abs(f.score - arr[i - 1].score) > 0.2
      );

    return hasUncertainty || hasComplexEnvironment;
  }

  private identifyDivergenceAreas(context: OptionGenerationContext): string[] {
    const areas: string[] = [];
  ...
  private createDivergentBrainstorming(context: OptionGenerationContext): Option {
    const divergenceAreas = this.identifyDivergenceAreas(context);

    return this.createOption(
      'Structured Divergent Brainstorming',
      'Organize collective ideation sessions that deliberately push beyond conventional boundaries. Use techniques that maximize idea diversity.',
      'process',
      [
  ...
  private createPerspectiveMultiplication(context: OptionGenerationContext): Option {
    // Use context to determine scope of perspective multiplication
    const hasLimitedPerspectives = context.pathMemory.constraints.some(c =>
      c.description.toLowerCase().includes('limited perspective')
    );
    ...
    return this.createOption(
      'Perspective Multiplication Protocol',
      'Systematically gather and amplify diverse viewpoints by engaging unlikely stakeholders and perspectives.',
      'relational',
      [
  ...
  private createCollectiveSensing(context: OptionGenerationContext): Option {
    // Use context to determine sensing priorities
    const uncertaintyLevel = context.pathMemory.constraints.filter(c =>
      c.description.toLowerCase().includes('uncertain')
    ).length;
    const sensingPriority =
      uncertaintyLevel > 2
        ? 'Prioritize early warning signals and emerging risks'
        : 'Balance between current state monitoring and future signals';
    return this.createOption(
      'Collective Sensing Network',
      'Establish a distributed sensing system where multiple observers track weak signals and emerging patterns.',
      'capability',
      [
  ...
  private createWisdomAggregation(context: OptionGenerationContext): Option {
    const currentConstraints = context.pathMemory.constraints.length;
    const aggregationScale =
      currentConstraints > 5
    ...
    return this.createOption(
      ...
      'Harness collective intelligence through structured aggregation of diverse insights, predictions, and solutions.',
      'process',
      [
