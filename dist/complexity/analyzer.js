...
export class HybridComplexityAnalyzer {
     ...
     */
    detectInteractingElementsNLP(analysis) {
        const entities = [
        ...
        const hasMultiple = analysis.topics.keywords.some(k => {
            ...
            return (lower === 'multiple' ||
                ...
                (lower.includes('multiple') && !lower.includes('multiples'))); // multiple but not multiples
        });
        // Check for interaction keywords in text
        const hasInteractionKeywords = analysis.topics.keywords.some(k => {
            const lower = k.toLowerCase();
            return (lower.includes('interact') ||
        ...
        const hasInteractionVerbs = relationships.relationships.some(r => ['interact', 'connect', 'integrate', 'communicate', 'coordinate', 'collaborate'].includes(r.verb.toLowerCase()));
        if (hasMultiple &&
    ...
    detectConflictsNLP(analysis) {
        ...
        if (analysis.topics.keywords.some(k => conflictKeywords.some(ck => k.toLowerCase().includes(ck)))) {
            return true;
        }
        return false;
    }
     ...
     */
    detectUncertaintyNLP(analysis) {
        // Check readability - complex text often indicates uncertainty
        if (analysis.readability.clarity === 'complex' ||
     ...
     */
    detectMultipleStakeholdersNLP(analysis) {
        ...
        if (analysis.topics.keywords.some(k => stakeholderKeywords.some(sk => k.toLowerCase().includes(sk)))) {
            return true;
        }
        return false;
    }
     ...
     */
    detectSystemComplexityNLP(analysis) {
     ...
     */
    detectTimePressureNLP(analysis) {
