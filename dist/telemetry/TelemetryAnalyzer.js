...
export class TelemetryAnalyzer {
    ...
    async getEventsForQuery(query) {
        ...
        if (query.techniques && query.techniques.length > 0) {
            events = events.filter(e => e.technique && query.techniques && query.techniques.includes(e.technique));
        }
     ...
     */
    updateMetrics(metrics, event, requestedMetrics) {
    ...
    generateSummary(events) {
        ...
        const averageSessionDuration = durationEvents.length > 0 ? totalDuration / durationEvents.length : 0;
        return {
     ...
     */
    finalizeTechniqueStats(stats, allEvents) {
     ...
     */
    findCommonCombinations(technique, events) {
        const combinations = new Map();
        // Group by session and find technique sequences
        const sessionTechniques = new Map();
        for (const event of events) {
        ...
        for (const [sid, techniques] of sessionTechniques) {
            if (techniques.includes(technique) && techniques.length > 1) {
                ...
                if (combo) {
                    ...
                    const effectivenessEvents = sessionEvents.filter(e => e.eventType === 'technique_complete' && e.metrics.effectiveness !== undefined);
                    if (effectivenessEvents.length > 0) {
                        const avgEffectiveness = effectivenessEvents.reduce((sum, e) => sum + (e.metrics.effectiveness || 0), 0) /
     ...
     */
    updateSessionAnalytics(analytics, event) {
        // Update time bounds
     ...
     */
    finalizeSessionAnalytics(analytics) {
        // Calculate duration
...
//# sourceMappingURL=TelemetryAnalyzer.js.map