...
export class PrivacyManager {
    ...
    sanitizeEvent(event) {
        ...
        const safeEvent = {
            ...
            anonymousSessionId: this.config.privacyMode === 'strict'
                ? 'aggregate'
...
`;
    ...
    static getConfigFromEnvironment() {
        return {
            ...
            level: PrivacyManager.parseChoice('TELEMETRY_LEVEL', ['basic', 'detailed', 'full'], 'basic'),
            storage: PrivacyManager.parseChoice('TELEMETRY_STORAGE', ['memory', 'filesystem', 'external'], 'memory'),
            ...
            privacyMode: PrivacyManager.parseChoice('TELEMETRY_PRIVACY_MODE', ['strict', 'balanced', 'minimal'], 'balanced'),
     ...
     */
    static parseChoice(name, allowed, fallback) {
        const raw = process.env[name];
        if (raw === undefined || raw === '')
            return fallback;
        if (allowed.includes(raw))
            return raw;
        process.stderr.write(`[Telemetry] ${name}="${raw}" is not one of ${allowed.join('|')}; using ${fallback}.\n`);
        return fallback;
    }
}
