...
export class ErrorFactory {
     ...
     */
    static stepOutOfOrder(technique, refusedStep, missingStep, localForm, planForm, guidance) {
        return new WorkflowError(ErrorCodes.STEP_OUT_OF_ORDER, `Step ${refusedStep} of ${technique} refused: step ${missingStep} has not been recorded ` +
     ...
     */
    static numberingRefused(technique, step, description, localForm, planForm) {
        return new WorkflowError(ErrorCodes.STEP_OUT_OF_ORDER, `Step ${step} of ${technique} refused: nothing was recorded. ${description}`, [
     ...
     */
    static stimulusRefused(technique, step, field, sent, assigned) {
        const listed = assigned.map(v => `"${v}"`).join(' / ');
        return new WorkflowError(ErrorCodes.STEP_OUT_OF_ORDER, `Step ${step} of ${technique} refused: nothing was recorded. It carries ${field} "${sent}", ` +
