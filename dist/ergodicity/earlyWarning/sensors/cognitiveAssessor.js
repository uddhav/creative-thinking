...
export class CognitiveAssessor extends Sensor {
    ...
    getRawReading(pathMemory, sessionData) {
        ...
        // Weighted average
        const overallRigidity = perspectiveRigidity * weights.perspectiveDiversity +
     ...
     */
    calculateCognitiveMetrics(pathMemory, sessionData) {
        return {
     ...
     */
    calculatePerspectiveDiversity(pathMemory) {
        ...
        const decisionPatterns = new Set(recentHistory.map(e => this.classifyDecisionPattern(e.decision)));
        ...
        const weights = recentHistory.length < 15
            ? { technique: 0.2, pattern: 0.4, option: 0.4 }
            : { technique: 0.33, pattern: 0.33, option: 0.34 };
        return Math.min(1, techniquesDiversity * weights.technique +
     ...
     */
    calculateLearningVelocity(sessionData) {
     ...
     */
    calculateMentalModelFlexibility(pathMemory) {
     ...
     */
    calculateCreativeDivergence(pathMemory) {
        if (pathMemory.pathHistory.length === 0) {
            return 1.0;
        }
     ...
     */
    adjustForCognitivePatterns(baseReading, pathMemory) {
     ...
     */
    detectPerspectiveNarrowing(pathMemory) {
     ...
     */
    detectLearningPlateau(sessionData) {
     ...
     */
    calculateInsightDensities(sessionData) {
        ...
        const firstHalfInsights = sessionData.insights.filter((_, i) => i < insightMidpoint).length;
     ...
     */
    detectFrameworkLockIn(pathMemory) {
        if (pathMemory.pathHistory.length < 10) {
            return false;
        }
        const recent = pathMemory.pathHistory.slice(-10);
     ...
     */
    detectRepetitiveThinking(pathMemory) {
        if (pathMemory.pathHistory.length < 10) {
            return false;
        }
        const recent = pathMemory.pathHistory.slice(-10);
     ...
     */
    classifyDecisionPattern(decision) {
        const lower = decision.toLowerCase();
        if (lower.includes('combine') || lower.includes('merge'))
            return 'combination';
        if (lower.includes('eliminate') || lower.includes('remove'))
            return 'elimination';
        if (lower.includes('reverse') || lower.includes('opposite'))
            return 'reversal';
        if (lower.includes('adapt') || lower.includes('modify'))
            return 'adaptation';
        return 'exploration';
    }
    /**
     ...
     */
    detectThinkingLoops(pathMemory) {
        if (pathMemory.pathHistory.length < 10)
            return 0;
        let loops = 0;
     ...
     */
    calculateMetricsImprovement(sessionData) {
        if (!sessionData.metrics)
            return 0.5;
        // Simple: check if metrics exist and are positive
        const hasPositiveMetrics = (sessionData.metrics.outputCompleteness || 0) > 0 ||
     ...
     */
    calculateApproachEvolution(sessionData) {
     ...
     */
    calculateAdaptationRate(pathMemory) {
        if (pathMemory.pathHistory.length < 5)
            return 0.5;
     ...
     */
    detectTechniqueFixation(pathMemory) {
        if (pathMemory.pathHistory.length < 8)
            return false;
     ...
     */
    detectCircularReasoning(pathMemory) {
        if (pathMemory.pathHistory.length < 10)
            return false;
        const decisions = pathMemory.pathHistory.map(e => this.normalizeDecision(e.decision));
     ...
     */
    detectConfirmationBias(pathMemory) {
        if (pathMemory.pathHistory.length < 10)
            return false;
        const recent = pathMemory.pathHistory.slice(-10);
        ...
        const closedOptions = recent.filter(e => e.optionsClosed.length > e.optionsOpened.length).length;
