...
export class StakesDiscovery {
     ...
     */
    generateStakesPrompt(sessionData, proposedAction) {
...
This is not a form. Think about YOUR specific situation and the risks YOU discovered.`;
     ...
     */
    generateRiskSpecificPrompts(indicators) {
        const prompts = [];
     ...
     */
    extractRelevantRisks(sessionData) {
        const risks = [];
        // From risk discovery
        if (sessionData.riskDiscoveryData?.risks) {
            ...
            if ('identifiedRisks' in discoveryRisks && Array.isArray(discoveryRisks.identifiedRisks)) {
                risks.push(...discoveryRisks.identifiedRisks.map((r) => typeof r === 'string' ? r : r.risk || JSON.stringify(r)));
            }
        }
     ...
     */
    extractRiskPhrasesFromText(text) {
        const risks = [];
     ...
     */
    validateStakes(declaration, metrics) {
        const missing = [];
        ...
        if (declaration.exitConditions) {
            const invalidConditions = declaration.exitConditions.filter(ec => !ec.measurable || ec.condition.length < 10);
     ...
     */
    generateHistoricalContext(declaration, indicators) {
        const examples = [];
        // Select relevant historical examples based on indicators
        if (indicators.some(i => i.includes('total commitment') || i.includes('all'))) {
            examples.push('LTCM (1998): "Our models account for everything" → Lost 90% in 4 months', 'Amaranth (2006): "Natural gas prices are predictable" → Lost $6.6B in 1 week');
        }
        if (indicators.some(i => i.includes('irreversible'))) {
            examples.push('Blockbuster (2000): "Streaming will never replace stores" → Bankrupt by 2010', 'Kodak (1975): "Digital will never match film quality" → Lost 90% of workforce');
        }
        if (indicators.some(i => i.includes('survival threat'))) {
            examples.push('Lehman Brothers (2008): "Real estate always recovers" → 158-year firm collapsed', 'Enron (2001): "Mark-to-market accounting is innovative" → Complete destruction');
        }
        if (examples.length === 0) {
            examples.push('General principle: Overconfidence in the face of identified risks is the most common path to ruin');
        }
...
//# sourceMappingURL=stakesDiscovery.js.map