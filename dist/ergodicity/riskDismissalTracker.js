...
export class RiskDismissalTracker {
     ...
     */
    trackAssessment(assessment, sessionData, proposedAction) {
     ...
     */
    extractRiskIndicators(assessment, proposedAction) {
        const indicators = [];
        ...
        const hasStandaloneAll = /\ball\s+(my|our|the)\s+(money|savings|resources|funds)/i.test(proposedAction);
        ...
        const hasGamblingContext = gamblingPhrases.some(phrase => actionLower.includes(phrase)) ||
     ...
     */
    checkRapidDismissals(sessionData) {
        if (!sessionData.history || sessionData.history.length < 2)
            return 0;
        let rapidCount = 0;
        ...
        for (let i = 1; i < recentHistory.length; i++) {
            ...
            if (timeDiff < this.RAPID_DISMISSAL_WINDOW) {
                if ('ruinAssessment' in current &&
                    current.ruinAssessment &&
                    'assessment' in current.ruinAssessment) {
                    const assessment = current.ruinAssessment.assessment;
     ...
     */
    findContradictions(sessionData) {
        ...
        sessionData.history.forEach((entry, index) => {
            if ('ruinAssessment' in entry &&
                ...
                const confidence = 'confidence' in assessment && typeof assessment.confidence === 'number'
                    ? assessment.confidence
                    : 1;
                const survivabilityThreatened = 'survivabilityThreatened' in assessment && assessment.survivabilityThreatened === true;
                ...
                if (confidence < 0.3 && survivabilityThreatened) {
                    contradictions.push(`Step ${index + 1}: Acknowledged survival threat with low confidence (${confidence})`);
                }
                // Irreversible action with casual dismissal
                if (isIrreversible && confidence < 0.5) {
                    contradictions.push(`Step ${index + 1}: Irreversible action dismissed with confidence ${confidence}`);
                }
            }
        });
        ...
        if (metrics && metrics.discoveredRiskIndicators.length > 0 && metrics.averageConfidence < 0.3) {
            const hasHighRisk = metrics.discoveredRiskIndicators.some(i => i.includes('irreversibility') || i.includes('survival'));
            if (hasHighRisk) {
                contradictions.push(`High-risk indicators (${metrics.discoveredRiskIndicators.join(', ')}) with low average confidence (${metrics.averageConfidence.toFixed(2)})`);
            }
        }
        return contradictions;
    }
     ...
     */
    detectHighStakesIndicators(sessionData) {
        const metrics = sessionData.riskEngagementMetrics;
        if (!metrics)
            return false;
        ...
        return highStakesIndicators.some(indicator => metrics.discoveredRiskIndicators.includes(indicator));
    }
    /**
     * Get severity based on consecutive dismissals
     */
    getConsecutiveSeverity(count) {
        if (count >= 7)
            return 'critical';
        if (count >= 5)
            return 'high';
     ...
     */
    generateBehavioralFeedback(patterns, metrics) {
    ...
    generateCriticalFeedback(pattern, metrics) {
...
This is not a procedural warning. Your pattern suggests overconfidence that could lead to the exact failures you discovered.`;
    ...
    generateHighFeedback(pattern, metrics) {
...
Please engage substantively with the risks YOU identified, not just acknowledge them.`;
    ...
    generateMediumFeedback(pattern, metrics) {
...
//# sourceMappingURL=riskDismissalTracker.js.map