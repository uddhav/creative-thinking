...
export class OptionEvaluator {
     ...
     */
    calculateFlexibilityGain(option, context) {
     ...
     */
    estimateImplementationCost(option, context) {
        const baseCost = 0.3;
        // Action count factor
        ...
        const cost = Math.min(0.9, baseCost + actionCost + prereqCost + categoryCost + constraintFactor) / 2;
    ...
    assessReversibility(option, context) {
        ...
        const avgCommitment = context.pathMemory.pathHistory.length > 0
        ...
        const createsConstraints = option.actions.some(a => a.toLowerCase().includes('commit') || a.toLowerCase().includes('permanent'));
     ...
     */
    calculateSynergies(option, context) {
        ...
        const addressedConstraints = context.pathMemory.constraints.filter(c => option.description.toLowerCase().includes(c.type) ||
    ...
    calculateOverallScore(metrics) {
        ...
        const score = metrics.flexibilityGain * weights.flexibilityGain +
     ...
     */
    determineRecommendation(score, context) {
     ...
     */
    generateReasoning(option, metrics) {
        const parts = [];
    ...
    hasPositiveSynergy(option, event) {
    ...
    hasConflict(option, constraint) {
        // Check if option might conflict with constraint
        if (!constraint.affectedOptions || !Array.isArray(constraint.affectedOptions)) {
            return false;
        }
        return constraint.affectedOptions.some((affected) => option.actions.some(action => action.toLowerCase().includes(affected.toLowerCase())));
    }
}
