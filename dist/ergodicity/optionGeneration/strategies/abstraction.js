...
export class AbstractionStrategy extends BaseOptionStrategy {
    strategyName = 'abstraction';
    description = 'Move up abstraction levels to discover new solution spaces and patterns';
    typicalFlexibilityGain = { min: 0.2, max: 0.5 };
    applicableCategories = ['conceptual', 'structural'];
    isApplicable(context) {
        // Abstraction is particularly useful when stuck in details
        const hasDetailedConstraints = context.pathMemory.constraints.some(c => c.description.length > 50 || // Detailed descriptions suggest specific constraints
            c.type === 'technical');
        const isStuckInSpecifics = context.currentFlexibility.flexibilityScore < 0.4 &&
    ...
    estimateEffort(option) {
        ...
        if (option.name.includes('Pattern'))
            return 'low';
        if (option.name.includes('Principle'))
            return 'low';
        return 'medium'; // Domain transfer and metaphors require more work
    }
    createPatternAbstractionOption(context) {
        ...
        return this.createOption('Apply Pattern Abstraction', `Step back from specific details to identify underlying patterns. Current situation shows patterns of: ${patterns.join(', ')}. By solving at the pattern level, multiple specific constraints can be addressed simultaneously.`, 'conceptual', actions, prerequisites);
    }
    createDomainTransferOption(context) {
        ...
        return this.createOption(`Apply ${primaryDomain.domain} Patterns`, `Transfer solutions from ${primaryDomain.domain} where they face similar challenges with ${primaryDomain.similarity}. For example, ${primaryDomain.example}. This opens new solution spaces not visible from within our domain.`, 'conceptual', actions, ['Study successful examples from target domain']);
    }
    createPrincipleExtractionOption(context) {
        ...
        return this.createOption('Extract and Apply Core Principles', `Identify the fundamental principles underlying successful decisions: ${principles.join(', ')}. Apply these principles in new ways to generate fresh options while maintaining what works.`, 'conceptual', actions, ['Review successful past decisions', 'Identify why they worked']);
    }
    createMetaphorOption(context) {
        ...
        return this.createOption(`Think Through ${metaphor.vehicle} Metaphor`, `View the current situation as ${metaphor.metaphor}. This metaphor suggests: ${metaphor.insights.join(', ')}. New options emerge by thinking about what would work in this metaphorical context.`, 'conceptual', actions, ['Team alignment on metaphor interpretation']);
    }
    ...
    extractPrinciples(context) {
        ...
        if (principles.length === 0) {
            principles.push('simplicity over complexity', 'adaptation over planning', 'learning over knowing');
        }
        return principles;
    }
    generateMetaphor(context) {
