...
export class CapabilityStrategy extends BaseOptionStrategy {
    strategyName = 'capability';
    description = 'Build flexibility by developing new capabilities and skills';
    typicalFlexibilityGain = { min: 0.2, max: 0.45 };
    applicableCategories = ['capability', 'process'];
    isApplicable(context) {
        // Look for capability gaps or skill constraints
        const hasCapabilityConstraints = context.pathMemory.constraints.some(c => c.description.toLowerCase().includes('skill') ||
        ...
        const hasLimitedTechniques = new Set(context.pathMemory.pathHistory.map(e => e.technique)).size < 3;
    ...
    estimateEffort(option) {
        if (option.name.includes('Quick'))
            return 'low';
        if (option.name.includes('Transfer') || option.name.includes('Tool'))
            return 'medium';
        return 'high'; // System building requires more effort
    }
    analyzeCapabilityGaps(context) {
        const analysis = {
            missingSkills: [],
            underutilizedSkills: [],
            emergingNeeds: [],
        ...
        if (context.currentFlexibility.flexibilityScore < 0.3) {
            ...
            analysis.emergingNeeds.push('opportunity recognition', 'strategic thinking', 'innovation methods');
        }
    ...
    createSkillDevelopmentOption(capabilities, context) {
        const prioritySkill = capabilities.missingSkills[0] || capabilities.emergingNeeds[0] || 'adaptive thinking';
        ...
        const hasTimeConstraints = context.pathMemory.constraints.some(c => c.description.toLowerCase().includes('time') ||
        ...
        return this.createOption(`Develop ${this.formatSkillName(prioritySkill)} Capability`, `Build ${prioritySkill} skills to address current constraints. This capability will open new solution approaches and reduce dependency on external expertise. ${isUrgent ? 'Focus on immediate application.' : 'Build deep expertise systematically.'}`, 'capability', actions, prerequisites);
    }
    createKnowledgeTransferOption(capabilities, context) {
        ...
        const hasCollaborationConstraints = context.pathMemory.constraints.some(c => c.description.toLowerCase().includes('team') ||
        ...
        const urgencyLevel = context.currentFlexibility.flexibilityScore < 0.4 ? 'rapid' : 'comprehensive';
        return this.createOption('Enable Knowledge Transfer', `Transfer ${transferableSkill} from those who have it to those who need it. This multiplies team capability without external investment. ${urgencyLevel === 'rapid' ? 'Focus on critical knowledge first.' : 'Build comprehensive knowledge base.'}`, 'capability', actions, ['Knowledge holder availability', 'Documentation infrastructure']);
    }
    createToolMasteryOption(capabilities, context) {
        const underusedTechnique = capabilities.underutilizedSkills
        ...
        return this.createOption(`Master ${this.formatSkillName(underusedTechnique)} Technique`, `Develop proficiency in ${underusedTechnique} to expand problem-solving toolkit. New techniques often reveal hidden options in familiar problems. ${needsQuickWins ? 'Fast-track learning for immediate impact.' : 'Build solid foundation for long-term mastery.'}`, 'capability', actions, needsQuickWins
            ? ['Quick reference guides', 'Immediate application opportunity']
            : ['Learning materials', 'Practice time', 'Patience with learning curve']);
    }
    createLearningSystemOption(context) {
        // Scale system complexity based on constraints and flexibility
        const hasResourceConstraints = context.pathMemory.constraints.some(c => c.type === 'resource' || c.description.toLowerCase().includes('budget'));
        const isLowFlexibility = context.currentFlexibility.flexibilityScore < 0.4;
        const actions = hasResourceConstraints || isLowFlexibility
            ? [
        ...
        const systemScale = hasResourceConstraints || isLowFlexibility ? 'lightweight' : 'comprehensive';
        return this.createOption(`Build ${systemScale === 'lightweight' ? 'Lightweight' : ''} Learning System`, `Create ${systemScale} approach to capturing and spreading learning. This compounds team capability over time and prevents repeated mistakes. ${systemScale === 'lightweight' ? 'Start small with minimal overhead.' : 'Build robust knowledge infrastructure.'}`, 'capability', actions, systemScale === 'lightweight'
            ? ['5 minutes per day commitment', 'Existing collaboration tools']
            : ['Team commitment to documentation', 'Simple tools for capture']);
    }
    inferRequiredSkill(constraintDescription) {
