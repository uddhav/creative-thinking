...
export class NeuralOptimizationStrategy extends BaseOptionStrategy {
    strategyName = 'neural_optimization';
    description = 'Create flexibility by optimizing neural states - switching between focused and diffuse thinking modes';
    typicalFlexibilityGain = { min: 0.15, max: 0.35 };
    applicableCategories = ['process', 'capability', 'conceptual'];
    isApplicable(context) {
        // Look for cognitive rigidity or stuck patterns
        const hasRigidPatterns = context.pathMemory.pathHistory.some(event => event.decision.toLowerCase().includes('stuck') ||
    ...
    estimateEffort(option) {
        if (option.name.includes('Activation'))
            return 'low';
        if (option.name.includes('Optimization'))
            return 'medium';
        if (option.name.includes('Rhythm'))
            return 'medium';
        return 'high'; // Redistribution requires more effort
    }
    hasRepetitivePatterns(context) {
        const recentDecisions = context.pathMemory.pathHistory.slice(-5);
        const uniquePatterns = new Set(recentDecisions.map(d => d.decision.split(' ').slice(0, 3).join(' ')));
        return uniquePatterns.size < recentDecisions.length * 0.6;
    }
    needsDMNActivation(context) {
    ...
    needsECNOptimization(context) {
        // Check if too scattered/unfocused
        const highVariability = context.pathMemory.flexibilityOverTime.length > 5 &&
            this.calculateVariability(context.pathMemory.flexibilityOverTime) > 0.3;
        return highVariability;
    }
    needsSwitchingRhythm(context) {
        // Check if stuck in one mode
        return (context.pathMemory.pathHistory.length > 10 &&
    ...
    createDMNOption(context) {
        // Use context to tailor DMN activation approach
        const flexibilityLevel = context.currentFlexibility.flexibilityScore;
        const urgencyNote = flexibilityLevel < 0.2
            ? 'Critical: Immediate DMN activation needed to restore flexibility'
            : 'Regular DMN activation sessions to maintain creative flow';
        return this.createOption('Default Mode Network Activation', 'Engage diffuse thinking by introducing unstructured exploration time. This activates the Default Mode Network to discover hidden connections and creative solutions.', 'process', [
    ...
    createECNOption(context) {
        ...
        const focusStrategy = variability > 0.4
            ? 'High variability detected - implement strict focus protocols'
            : 'Maintain balanced focus with periodic reviews';
        return this.createOption('Executive Control Network Optimization', 'Enhance focused problem-solving by structuring decision-making processes. This optimizes the Executive Control Network for systematic analysis.', 'process', [
    ...
    createSwitchingOption(context) {
        // Use context to design optimal switching rhythm
        const sessionLength = context.pathMemory.pathHistory.length;
        const rhythmDesign = sessionLength > 20
            ? 'Extended work requires more frequent mode switches'
            : 'Standard pomodoro-style switching rhythm';
        return this.createOption('Neural Network Switching Rhythm', 'Develop a rhythm between focused and diffuse thinking modes. This creates cognitive flexibility through intentional mode switching.', 'capability', [
    ...
    createLoadRedistributionOption(context) {
        const currentConstraints = context.pathMemory.constraints.filter(c => c.strength > 0.5).length;
        const redistributionPriority = currentConstraints > 4
        ...
        return this.createOption('Cognitive Load Redistribution', 'Redistribute cognitive load by externalizing constraints and decisions. This frees up mental resources for creative problem-solving.', 'conceptual', [
...
//# sourceMappingURL=neuralOptimization.js.map