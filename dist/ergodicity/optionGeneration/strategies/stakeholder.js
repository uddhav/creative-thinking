...
export class StakeholderStrategy extends BaseOptionStrategy {
    strategyName = 'stakeholder';
    description = 'Generate options by considering different stakeholder perspectives and needs';
    typicalFlexibilityGain = { min: 0.15, max: 0.4 };
    applicableCategories = ['relational', 'process'];
    isApplicable(context) {
        // Look for stakeholder-related constraints or decisions
        const hasStakeholderConstraints = context.pathMemory.constraints.some(c => c.type === 'relational' ||
        ...
        const hasRelationalCommitments = context.pathMemory.pathHistory.some(e => e.decision.toLowerCase().includes('agreed') ||
    ...
    estimateEffort(option) {
        if (option.name.includes('Perspective'))
            return 'low';
        if (option.name.includes('Reframe'))
            return 'medium';
    ...
    createPerspectiveShiftOption(stakeholders, context) {
        ...
        const pastEngagements = context.pathMemory.pathHistory.filter(e => e.decision.toLowerCase().includes('stakeholder') ||
            e.decision.toLowerCase().includes('perspective')).length;
        ...
        const prerequisites = engagementDepth === 'rapid'
        ...
        const approachNote = engagementDepth === 'rapid'
            ? ' Quick wins through fresh perspective.'
            : engagementDepth === 'targeted'
                ? ' Focus on previously missed insights.'
                : ' Deep understanding drives breakthrough solutions.';
        return this.createOption(`Adopt ${flexibleStakeholder.name} Perspective`, `Shift primary viewpoint from internal to ${flexibleStakeholder.name} perspective. They ${flexibleStakeholder.currentPosition.toLowerCase()}. Designing from their viewpoint may reveal simpler, more flexible solutions.${approachNote}`, 'relational', actions, prerequisites);
    }
    createStakeholderReframeOption(stakeholders, context) {
        const influentialStakeholder = stakeholders.find(s => s.influence === 'high') || stakeholders[0];
        // Check communication history and constraint types
        const pastCommunications = context.pathMemory.pathHistory.filter(e => e.decision.toLowerCase().includes('communicate') ||
            e.decision.toLowerCase().includes('frame')).length;
        const hasRelationalConstraints = context.pathMemory.constraints.some(c => c.type === 'relational' || c.description.toLowerCase().includes('perception'));
        // Adjust framing strategy based on context
        const framingStrategy = pastCommunications > 2 ? 'innovative' : hasRelationalConstraints ? 'corrective' : 'strategic';
        const actions = [
        ...
        const prerequisites = framingStrategy === 'corrective'
        ...
        const strategyNote = framingStrategy === 'corrective'
        ...
        return this.createOption('Reframe for Key Stakeholders', `Reposition the current challenge in terms that resonate with ${influentialStakeholder.name}. Transform constraints into opportunities by aligning with what they value most.${strategyNote}`, 'relational', actions, prerequisites);
    }
    createCoalitionOption(stakeholders, context) {
        const potentialAllies = stakeholders.filter(s => s.flexibility !== 'low' && s.influence !== 'low');
        if (potentialAllies.length < 2)
            return null;
        // Check coalition-building history and political constraints
        const pastCoalitionAttempts = context.pathMemory.pathHistory.filter(e => e.decision.toLowerCase().includes('coalition') || e.decision.toLowerCase().includes('unite')).length;
        const hasPoliticalConstraints = context.pathMemory.constraints.some(c => c.type === 'relational' || c.description.toLowerCase().includes('political'));
        // Adjust coalition strategy based on context
        const coalitionApproach = pastCoalitionAttempts > 1 ? 'selective' : hasPoliticalConstraints ? 'careful' : 'ambitious';
        const actions = [
        ...
        const prerequisites = coalitionApproach === 'careful'
        ...
        const approachNote = coalitionApproach === 'careful'
        ...
        return this.createOption('Build Stakeholder Coalition', `Unite ${potentialAllies.map(s => s.name).join(' and ')} around ${coalitionApproach === 'selective' ? 'unique' : 'shared'} interests. Combined influence can overcome individual constraints and create new possibilities.${approachNote}`, 'relational', actions, prerequisites);
    }
    createValueRedistributionOption(stakeholders, context) {
        ...
        const pastRedistributions = context.pathMemory.pathHistory.filter(e => e.decision.toLowerCase().includes('value') ||
            e.decision.toLowerCase().includes('redistribute')).length;
        ...
        const redistributionStrategy = pastRedistributions > 1 ? 'innovative' : hasResourceConstraints ? 'efficient' : 'expansive';
        const actions = [
        ...
        const prerequisites = redistributionStrategy === 'innovative'
        ...
        const strategyNote = redistributionStrategy === 'innovative'
        ...
        return this.createOption('Redistribute Stakeholder Value', `Redesign how value flows between stakeholders. By better serving currently underserved groups, new options and alliances become possible. Focus on ${redistributionStrategy === 'efficient' ? 'doing more with less' : 'expanding the pie, not just dividing it'}.${strategyNote}`, 'relational', actions, prerequisites);
    }
    extractStakeholderMentions(text) {
        const stakeholders = [];
    ...
    inferPosition(stakeholderName, context) {
        // Infer stakeholder position from context
        const constraints = context.pathMemory.constraints.filter(c => c.description.toLowerCase().includes(stakeholderName.toLowerCase()));
    ...
    assessStakeholderFlexibility(name, context) {
        // Assess based on how often they appear in rigid constraints
        const rigidMentions = context.pathMemory.constraints.filter(c => c.description.toLowerCase().includes(name.toLowerCase()) && c.reversibilityCost > 0.6).length;
        if (rigidMentions > 2)
            return 'low';
        if (rigidMentions > 0)
            return 'medium';
    ...
    assessStakeholderInfluence(name, context) {
        // Assess based on appearance in high-commitment decisions
        const highCommitmentMentions = context.pathMemory.pathHistory.filter(e => e.decision.toLowerCase().includes(name.toLowerCase()) && e.commitmentLevel > 0.5).length;
        if (highCommitmentMentions > 2)
            return 'high';
        if (highCommitmentMentions > 0)
            return 'medium';
        // Special cases
        if (name.includes('Leadership') || name.includes('Management'))
            return 'high';
