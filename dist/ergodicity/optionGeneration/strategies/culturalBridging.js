...
export class CulturalBridgingStrategy extends BaseOptionStrategy {
    strategyName = 'cultural_bridging';
    description = 'Create flexibility by bridging different cultural frameworks, contexts, and worldviews';
    typicalFlexibilityGain = { min: 0.25, max: 0.45 };
    applicableCategories = ['relational', 'conceptual', 'structural'];
    isApplicable(context) {
        // Look for cultural, contextual, or framework conflicts
        const hasFrameworkConflict = context.pathMemory.constraints.some(c => c.description.toLowerCase().includes('conflict') ||
            c.description.toLowerCase().includes('incompatible') ||
            c.description.toLowerCase().includes('different approach'));
        const hasMultipleStakeholders = context.pathMemory.constraints.filter(c => c.description.toLowerCase().includes('stakeholder') ||
            c.description.toLowerCase().includes('team') ||
            c.description.toLowerCase().includes('department')).length > 1;
        const hasContextualRigidity = context.currentFlexibility.flexibilityScore < 0.4 &&
            context.pathMemory.pathHistory.some(e => e.decision.toLowerCase().includes('must') || e.decision.toLowerCase().includes('only way'));
    ...
    estimateEffort(option) {
        if (option.name.includes('Bridge Concepts'))
            return 'low';
        if (option.name.includes('Translation'))
            return 'medium';
        if (option.name.includes('Synthesis') || option.name.includes('Parallel'))
            return 'high';
        return 'medium';
    }
    hasMultipleValidApproaches(context) {
    ...
    createSynthesisFramework(context) {
        const frameworks = this.identifyFrameworks(context);
        const frameworkCount = frameworks.length;
        const complexityNote = frameworkCount > 2
            ? `Complex synthesis required across ${frameworkCount} frameworks`
            : 'Focused synthesis between primary frameworks';
        return this.createOption('Create Synthesis Framework', 'Develop a meta-framework that incorporates strengths from different cultural or methodological approaches, creating a flexible hybrid.', 'conceptual', [
    ...
    createTranslationInterface(context) {
        // Use context to assess translation complexity
        const stakeholderCount = context.pathMemory.constraints.filter(c => c.description.toLowerCase().includes('stakeholder')).length;
        const interfaceScope = stakeholderCount > 3
            ? 'Multi-stakeholder translation framework required'
            : 'Focused bilateral translation interfaces';
        return this.createOption('Build Translation Interfaces', 'Create interfaces that translate between different cultural or contextual frameworks, enabling collaboration without forcing uniformity.', 'relational', [
    ...
    createParallelPaths(context) {
        // Use context to determine parallel path strategy
        const highCommitment = context.pathMemory.pathHistory.filter(e => e.commitmentLevel > 0.7).length;
        const pathStrategy = highCommitment > 2
            ? 'Create escape routes from high-commitment paths'
            : 'Design flexible convergence points between paths';
        return this.createOption('Implement Parallel Cultural Paths', 'Allow different approaches to coexist and proceed in parallel, converging only where necessary. This preserves cultural integrity while enabling progress.', 'structural', [
    ...
    createBridgeConcepts(context) {
        const constraints = this.getRelevantConstraints(context).slice(0, 3);
        const bridgeComplexity = constraints.length > 2
            ? `Address ${constraints.length} key constraints through bridge concepts`
            : 'Focus on primary constraint bridges';
        return this.createOption('Develop Bridge Concepts', 'Identify or create concepts that naturally exist in multiple frameworks, serving as connection points for integration.', 'conceptual', [
...
//# sourceMappingURL=culturalBridging.js.map