...
export const BUILTIN_PERSONAS = {
    rory_sutherland: {
        ...
        perspective: 'Human behavior is irrational and that irrationality is a feature, not a bug. The best solutions often feel wrong to economists.',
        techniqueBias: {
    ...
    rich_hickey: {
        ...
        perspective: 'Simplicity is a prerequisite for reliability. Complexity is the enemy. Complecting things together creates the hardest problems to solve.',
        techniqueBias: {
    ...
    joe_armstrong: {
        ...
        perspective: 'The world is concurrent. Things fail. Build systems that embrace failure and recover gracefully. Isolation is the key to reliability.',
        techniqueBias: {
            biomimetic_path: 0.9,
    ...
    tarantino: {
        ...
        perspective: 'Convention is boring. Break chronology, steal from the best, combine genres nobody thought could mix, and make the mundane extraordinary through sheer audacity.',
        techniqueBias: {
            random_entry: 0.95,
            disney_method: 0.85,
    ...
    security_engineer: {
        ...
        perspective: 'Every system will be attacked. Think like an adversary. Defense in depth. Trust nothing, verify everything. The question is not if, but when.',
        techniqueBias: {
    ...
    veritasium: {
        ...
        perspective: 'The most effective way to learn is to first confront your misconceptions. The counterintuitive truth is more memorable than the expected answer.',
        techniqueBias: {
    ...
    design_thinker: {
        ...
        perspective: 'Every problem is ultimately a human problem. Start with people, not technology. Prototype fast, fail cheap, iterate relentlessly.',
        techniqueBias: {
    ...
    nassim_taleb: {
        ...
        name: 'Nassim Taleb',
        tagline: 'Antifragile',
        perspective: 'Some things benefit from shocks. The key question is not "will this survive?" but "will this get stronger from stress?" Beware of fragility hiding behind complexity.',
        techniqueBias: {
 ...
 */
function sanitizePersonaString(str, maxLength = MAX_PERSONA_STRING_LENGTH) {
 ...
 */
function sanitizePersonaDefinition(def) {
    return {
        ...def,
 ...
 */
export function loadExternalPersonas(path) {
    try {
 ...
 */
function isValidPersonaDefinition(obj) {
    ...
    if (!VALID_PREFERRED_OUTCOMES.includes(def.preferredOutcome)) {
        return false;
    }
    // Validate techniqueBias is a non-null, non-array object with valid technique keys
    if (typeof def.techniqueBias !== 'object' ||
    ...
    const validTechniques = new Set(ALL_LATERAL_TECHNIQUES);
    ...
    for (const field of ['keyPrinciples', 'evaluationCriteria', 'challengeQuestions']) {
        const arr = def[field];
    ...
    if (typeof def.thinkingStyle !== 'object' || def.thinkingStyle === null)
        return false;
    const style = def.thinkingStyle;
    if (typeof style.approach !== 'string' || style.approach.length === 0)
        return false;
    if (!Array.isArray(style.strengths) || style.strengths.length === 0)
        return false;
    if (!Array.isArray(style.blindSpots))
        return false; // blindSpots can be empty
    return true;
}
