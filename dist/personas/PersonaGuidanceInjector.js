...
export class PersonaGuidanceInjector {
     ...
     */
    static createStepContext(persona, step, _totalSteps) {
     ...
     */
    static injectGuidance(originalGuidance, persona, step, totalSteps) {
     ...
     */
    static createDebateSynthesisHeader(personas) {
        const lines = personas.map(p => `• **${p.name}** (${p.tagline}): ${p.perspective.split('.')[0]}.`);
...
//# sourceMappingURL=PersonaGuidanceInjector.js.map