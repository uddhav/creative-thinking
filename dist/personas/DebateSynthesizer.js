...
export class DebateSynthesizer {
     ...
     */
    synthesize(topic, positions, personas) {
        ...
        const actionableSynthesis = this.generateSynthesis(topic, positions, agreements, disagreements, blindSpots);
        return {
            topic,
            positions,
            agreements,
    ...
    findAgreements(positions) {
        ...
        for (let i = 0; i < positions.length; i++) {
            for (let j = i + 1; j < positions.length; j++) {
                const shared = this.findOverlappingThemes(positions[i].keyArguments, positions[j].keyArguments);
        ...
        for (let i = 0; i < positions.length; i++) {
            for (let j = i + 1; j < positions.length; j++) {
                if (positions[i].proposedSolution !== positions[j].proposedSolution) {
                    disagreements.push(`${positions[i].personaName} proposes "${positions[i].proposedSolution}" vs ${positions[j].personaName} proposes "${positions[j].proposedSolution}"`);
                }
            }
        }
     ...
     */
    findBlindSpots(positions, personas) {
        const blindSpots = [];
     ...
     */
    generateSynthesis(topic, positions, agreements, disagreements, blindSpots) {
        const lines = [];
        ...
        if (positions.length > 0) {
            lines.push(`Consider the ${agreements.length} agreement(s) as starting points, ` +
     ...
     */
    findOverlappingThemes(args1, args2) {
        const overlaps = [];
        ...
        for (const arg1 of args1) {
            ...
            if (words1.size < 2)
                continue; // Can't match 2+ words with fewer than 2
...
//# sourceMappingURL=DebateSynthesizer.js.map