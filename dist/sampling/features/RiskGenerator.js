...
export class RiskGenerator {
     ...
     */
    async generateRisks(solution, context, domain) {
        try {
            ...
            return this.handleError(solution, error);
        }
    }
    /**
     * Generate risks for multiple solutions
     */
    async generateBatchRisks(solutions, domain) {
        const assessments = await Promise.allSettled(solutions.map(({ solution, context }) => this.generateRisks(solution, context, domain)));
...
Be thorough but practical. Focus on risks that could materially impact success.`;
     ...
     */
    buildUserPrompt(solution, context) {
...
End with an OVERALL RISK ASSESSMENT: [low/medium/high/critical]`;
     ...
     */
    parseRiskAssessment(aiResponse, solution) {
        const assessment = {
            risks: [],
            overallRisk: 'medium',
        };
        try {
            ...
            assessment.risks = [...criticalRisks, ...majorRisks, ...moderateRisks, ...edgeCases].slice(0, 10);
            // Extract overall risk level
            const overallMatch = aiResponse.match(/OVERALL RISK (?:ASSESSMENT|LEVEL):\s*\[?(low|medium|high|critical)\]?/i);
            if (overallMatch) {
                assessment.overallRisk = overallMatch[1].toLowerCase();
            }
     ...
     */
    extractRisks(sectionText, defaultSeverity, defaultLikelihood) {
        if (!sectionText)
            return [];
        const risks = [];
     ...
     */
    parseRiskBlock(block, defaultSeverity, defaultLikelihood) {
     ...
     */
    fallbackRiskAssessment(_solution) {
        return {
            risks: [
                {
...
//# sourceMappingURL=RiskGenerator.js.map