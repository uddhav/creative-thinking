...
export class IdeaEnhancer {
     ...
     */
    async enhance(idea, technique, context) {
        try {
            ...
            return this.handleError(idea, error);
        }
    }
    /**
     * Enhance multiple ideas in batch
     */
    async enhanceBatch(ideas, context) {
        const enhancements = await Promise.allSettled(ideas.map(({ idea, technique }) => this.enhance(idea, technique, context)));
...
Keep the enhancement creative yet actionable.`;
     ...
     */
    parseEnhancement(originalIdea, aiResponse) {
        const enhancement = {
        ...
        try {
            ...
            if (sections['IMPLEMENTATION CONSIDERATIONS']) {
                enhancement.implementation = this.extractBulletPoints(sections['IMPLEMENTATION CONSIDERATIONS']);
            }
     ...
     */
    getTechniqueSpecificPrompt(technique) {
        const prompts = {
...
//# sourceMappingURL=IdeaEnhancer.js.map