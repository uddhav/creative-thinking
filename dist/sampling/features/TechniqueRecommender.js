...
export class TechniqueRecommender {
     ...
     */
    async recommendNextTechnique(state) {
        try {
     ...
     */
    async recommendMultipleTechniques(state, count = 3) {
        try {
            ...
            return [
                ...
                ...(primary.alternatives?.slice(0, count - 1).map(alt => ({
                    ...
                    expectedBenefits: this.getExpectedBenefits(alt.technique),
                    confidence: 0.7,
                })) || []),
            ];
        }
...
Focus on maximizing creative output and maintaining session flow.`;
     ...
     */
    parseRecommendation(aiResponse, state) {
        ...
        try {
            ...
            if (recommendation.expectedBenefits.length === 0) {
                recommendation.expectedBenefits = this.getExpectedBenefits(recommendation.technique);
            }
        }
        catch (error) {
    ...
    isValidTechnique(technique) {
        return this.allTechniques.includes(technique);
    }
    /**
     * Select fallback technique based on state
     */
    selectFallbackTechnique(state) {
        ...
        if (unused.length === 0) {
            ...
            if (state.ideasGenerated < 5)
                return 'random_entry';
            if (state.flexibilityScore && state.flexibilityScore < 5)
                return 'po';
     ...
     */
    generateAlternatives(state, count, exclude) {
        const alternatives = [];
        const unused = this.allTechniques.filter(t => !state.techniquesUsed.includes(t) && t !== exclude);
        ...
        for (const priority of priorities) {
            if (unused.includes(priority.technique) && alternatives.length < count) {
    ...
    fallbackRecommendation(state) {
        ...
        return {
            ...
            expectedBenefits: this.getExpectedBenefits(technique),
