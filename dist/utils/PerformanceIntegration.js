 ...
 */
export function wrapComplexityAnalyzer(analyzer) {
    ...
    return new Proxy(analyzer, {
        get(target, prop, receiver) {
            ...
            if (prop === 'analyze' && typeof value === 'function') {
                return (text, useCache) => {
                    const fn = value;
                    return performanceProfiler.measureSync('complexity_analysis', () => fn.call(target, text, useCache), { textLength: text.length });
                };
            }
            return value;
        },
 ...
 */
export function wrapErgodicityManager(manager) {
    if (!performanceProfiler.isEnabled())
        return manager;
    // Use a Proxy to intercept method calls
    return new Proxy(manager, {
        get(target, prop, receiver) {
            ...
            if (prop === 'recordThinkingStep' && typeof value === 'function') {
                return async (technique, step, output, impact, session) => {
                    const fn = value;
                    return performanceProfiler.measureAsync('ergodicity_record_step', () => fn.call(target, technique, step, output, impact || {}, session), { technique, step });
                };
            }
            if (prop === 'analyzeEscapeVelocity' && typeof value === 'function') {
                return (sessionData) => {
                    const fn = value;
                    return performanceProfiler.measureSync('ergodicity_analyze_escape', () => fn.call(target, sessionData));
                };
            }
            return value;
        },
 ...
 */
export function wrapSessionManager(manager) {
    if (!performanceProfiler.isEnabled())
        return manager;
    // Use a Proxy to intercept method calls
    return new Proxy(manager, {
        get(target, prop, receiver) {
            ...
            if (prop === 'saveSessionToPersistence' && typeof value === 'function') {
                return async (sessionId) => {
                    const fn = value;
                    return performanceProfiler.measureAsync('session_save', () => fn.call(target, sessionId), { sessionId });
                };
            }
            if (prop === 'loadSessionFromPersistence' && typeof value === 'function') {
                return async (sessionId) => {
                    const fn = value;
                    return performanceProfiler.measureAsync('session_load', () => fn.call(target, sessionId), { sessionId });
                };
            }
            return value;
        },
 ...
 */
export function wrapTechniqueExecution(techniqueName, fn) {
    if (!performanceProfiler.isEnabled())
        return fn;
    return ((...args) => {
...
export function logPerformanceReport() {
    ...
    if (report.metrics.length > 0) {
        ...
        report.metrics.slice(0, 5).forEach(m => {
            console.error(`  ${m.operationName}: ${m.count} calls, ` +
...
//# sourceMappingURL=PerformanceIntegration.js.map