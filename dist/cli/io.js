 ...
 */
export async function readStdinJSON(stream = process.stdin, firstByteTimeoutMs = STDIN_FIRST_BYTE_TIMEOUT_MS) {
    if (stream.isTTY)
        return null;
    const chunks = [];
    await new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
            if (sawFirstChunk)
                return;
            cleanup();
            reject(new Error(`stdin stayed open and sent nothing for ${firstByteTimeoutMs}ms. ` +
