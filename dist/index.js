...
export class LateralThinkingServer {
    ...
    async processLateralThinking(input) {
        try {
            ...
            return this.responseBuilder.buildErrorResponse(error instanceof Error ? error : new Error('Unknown error'), 'execution');
        }
    }
    /**
    ...
    async discoverTechniques(input) {
        try {
            ...
            return this.responseBuilder.buildErrorResponse(error instanceof Error ? error : new Error('Unknown error'), 'discovery');
        }
    }
    /**
    ...
    async planThinkingSession(input) {
        try {
            ...
            return this.responseBuilder.buildErrorResponse(error instanceof Error ? error : new Error('Unknown error'), 'planning');
        }
    }
    /**
    ...
    async handleSessionOperation(input) {
        try {
            switch (input.sessionOperation) {
                ...
                default:
                    throw new Error(`Unknown session operation: ${input.sessionOperation}`);
            }
        }
        catch (error) {
            return this.responseBuilder.buildErrorResponse(error instanceof Error ? error : new Error('Unknown error'), 'session');
        }
    }
...
const DISCOVER_TECHNIQUES_TOOL = {
    ...
    description: 'Analyzes a problem and recommends appropriate lateral thinking techniques based on problem characteristics and desired outcomes',
    inputSchema: {
        type: 'object',
        properties: {
            problem: {
...
const PLAN_THINKING_SESSION_TOOL = {
    ...
    description: 'Creates a structured workflow for applying one or more lateral thinking techniques to a problem',
    inputSchema: {
        type: 'object',
        properties: {
            problem: {
...
const EXECUTE_THINKING_STEP_TOOL = {
    ...
    description: 'Executes a single step in the lateral thinking process, maintaining session state and providing structured guidance',
    inputSchema: {
        type: 'object',
        properties: {
            planId: { type: 'string' },
