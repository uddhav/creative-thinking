...
export class ScamperHandler extends BaseTechniqueHandler {
    actions = {
        ...
        eliminate: {
            ...
            riskQuestion: 'What is permanently lost through elimination — and what does keeping it continue to cost?',
    ...
    extractInsights(history) {
        ...
        for (let step = 1; step <= this.actionOrder.length; step++) {
            ...
            if (!entry)
                continue;
            const action = entry.scamperAction ?? this.actionOrder[step - 1];
            const declaredIndex = this.actionOrder.indexOf(action);
            const stepName = this.steps[declaredIndex >= 0 ? declaredIndex : step - 1]?.name ?? `Step ${step}`;
