...
export class SteelmanRedTeamHandler extends BaseTechniqueHandler {
    ...
    getTechniqueInfo() {
        return {
            ...
            emoji: '🥊',
            totalSteps: 7,
            description: 'Builds the opposing case until its holders would sign it, then attacks the plan from a named adversary who wants it to fail',
            focus: 'Occupy a position that is not yours, cooperatively and then hostilely',
            enhancedFocus: 'Gated at both halves: a caricatured opponent fails the Turing test, and findings that could not have changed the decision fail the consequence check',
            parallelSteps: {
                ...
                description: 'Sequential, and the gates are the reason: the Turing test can only judge a case already built, and the consequence check can only judge findings already produced',
            },
            reflexivityProfile: {
...
//# sourceMappingURL=SteelmanRedTeamHandler.js.map