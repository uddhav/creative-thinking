...
const MODEL_BY_ID = new Map(MENTAL_MODELS.map((m) => [m.id, m]));
export class MentalModelsHandler extends BaseTechniqueHandler {
    ...
    getTechniqueInfo() {
        return {
            ...
            emoji: '🧰',
            totalSteps: 7,
            description: 'Forces a problem through named disciplinary lenses — physics, biology, psychology, economics — to escape the one-model trap',
            focus: 'To the man with only a hammer every problem looks like a nail; carry multiple tools instead',
            enhancedFocus: 'Surfaces where disciplines disagree and demands synthesis rather than retreat into one orthodoxy',
            parallelSteps: {
                canParallelize: true,
                description: 'The four disciplinary lenses (steps 2-5) are independent and can be applied in parallel after framing',
            },
            reflexivityProfile: {
    ...
    getStepInfo(step) {
        if (step < 1 || step > this.steps.length) {
            throw new ValidationError(ErrorCode.INVALID_STEP, `Invalid step ${step} for Latticework of Mental Models. Valid steps are 1-${this.steps.length}`, 'step', { received: step, expected: `1-${this.steps.length}` });
        }
...
//# sourceMappingURL=MentalModelsHandler.js.map