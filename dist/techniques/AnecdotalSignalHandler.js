...
Output: Complete strategic response plan with monitoring systems`,
        ...
        return (guidanceTemplates[step] || `Complete the Anecdotal Signal Detection process for: "${problem}"`);
    }
    ...
    extractInsights(history) {
        ...
        for (let step = 1; step <= totalSteps; step++) {
            ...
            if (step === 2 && Array.isArray(entryObj.signals)) {
                const signals = entryObj.signals.filter((s) => typeof s === 'object' && s !== null);
                const strongSignals = signals.filter(s => s.signalStrength === 'strong' || s.signalStrength === 'critical');
                if (strongSignals.length > 0) {
                    ...
                    insights.push(`${stepName}: ${strongSignals.length} of ${signals.length} signals rated strong or critical — ${described}.${remainder}`);
                }
            }
            if (step === 3) {
            ...
            if (step === 4 && Array.isArray(entryObj.earlyWarnings)) {
                ...
                if (entryObj.earlyWarnings.length > 0) {
                    insights.push(`${stepName}: ${entryObj.earlyWarnings.length} early warning signals extracted — ${warnings}`);
                }
            }
            if (step === 5 && Array.isArray(entryObj.scalingScenarios)) {
                ...
                const mainstream = entryObj.scalingScenarios.filter((s) => {
                ...
                if (mainstream.length > 0) {
                    insights.push(`${stepName}: ${mainstream.length} projection(s) cross mainstream adoption (>25%)`);
                }
            }
            if (step === 6) {
