    ...
    getTechniqueInfo() {
        return {
            ...
            parallelSteps: {
                ...
                description: 'Must be executed sequentially: movement requires provocation, development requires movement insights',
            },
        };
    }
    getStepInfo(step) {
        ...
        if (step < 1 || step > steps.length) {
            throw new ValidationError(ErrorCode.INVALID_STEP, `Invalid step ${step} for PO technique. Valid steps are 1-${steps.length}`, 'step', { providedStep: step, validRange: `1-${steps.length}` });
        }
        return steps[step - 1];
    }
