...
export class CompetingHypothesesHandler extends BaseTechniqueHandler {
    ...
    getTechniqueInfo() {
        return {
            ...
            emoji: '⚖️',
            totalSteps: 8,
            description: 'Systematic evaluation of multiple explanations using evidence matrices and Bayesian reasoning',
            focus: 'Rigorous hypothesis testing to reduce confirmation bias and handle uncertainty',
            enhancedFocus: 'Prevents premature convergence and quantifies confidence in complex decisions',
            parallelSteps: {
...
Output: Complete decision package with conclusion, confidence, actions, and monitoring plan`,
    ...
    validateMatrix(matrix) {
        if (!matrix || typeof matrix !== 'object')
            return false;
        const m = matrix;
        if (!m.hypotheses || !Array.isArray(m.hypotheses))
            return false;
        if (!m.evidence || !Array.isArray(m.evidence))
            return false;
        if (!m.ratings || typeof m.ratings !== 'object' || m.ratings === null)
            return false;
     ...
     */
    describeMatrix(stepName, matrix) {
        const described = [];
        ...
        if (hypotheses.length > 0) {
            described.push(`${stepName}: Evaluating ${hypotheses.length} competing hypotheses — ${hypotheses.join(', ')}`);
        }
        if (evidence.length > 0) {
            described.push(`${stepName}: ${evidence.length} pieces of evidence mapped — ${evidence.join(', ')}`);
        }
        const ratings = Object.entries(matrix.ratings ?? {});
        if (ratings.length > 0) {
            described.push(`${stepName}: Compatibility ratings — ${ratings
        ...
        if (diagnostic.length > 0) {
            ...
            described.push(`${stepName}: Diagnostic value, most discriminating first — ${ranked
                .map(([item, value]) => `${item} ${value}`)
                .join(', ')}`);
        }
        const carriedProbabilities = Object.entries(matrix.probabilities ?? {});
        if (carriedProbabilities.length > 0) {
            described.push(`${stepName}: Probabilities carried in the matrix — ${carriedProbabilities
    ...
    describeProbabilities(stepName, probabilities) {
    ...
    createEmptyMatrix(hypotheses, evidence) {
        const matrix = {
            hypotheses,
            evidence,
            ratings: {},
...
//# sourceMappingURL=CompetingHypothesesHandler.js.map