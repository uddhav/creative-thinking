...
export class LinguisticForensicsHandler extends BaseTechniqueHandler {
    ...
    getTechniqueInfo() {
        return {
            ...
            parallelSteps: {
                ...
                description: 'Sequential analysis required as each step builds on previous linguistic findings',
            },
            reflexivityProfile: {
    ...
    getStepInfo(step) {
        if (step < 1 || step > this.steps.length) {
            throw new ValidationError(ErrorCode.INVALID_STEP, `Invalid step ${step} for Linguistic Forensics. Valid steps are 1-${this.steps.length}`, 'step', { received: step, expected: `1-${this.steps.length}` });
        }
...
Output: Complete linguistic forensics report with coherence verdict and confidence level`,
    ...
    validateStep(step, data) {
        ...
        if (step === 3 && data && typeof data === 'object' && 'pronounRatios' in data) {
            const pronounRatios = data.pronounRatios;
            // Validate pronoun ratios are proper numbers
            if (typeof pronounRatios === 'object' && pronounRatios !== null) {
                return Object.values(pronounRatios).every(ratio => typeof ratio === 'number' && ratio >= 0 && ratio <= 1);
            }
            return false;
        }
        if (step === 6 && data && typeof data === 'object' && 'coherenceScore' in data) {
            const coherenceScore = data.coherenceScore;
    ...
    getLinguisticMarkers() {
        return this.linguisticMarkers;
    }
}
//# sourceMappingURL=LinguisticForensicsHandler.js.map