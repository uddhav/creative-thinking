            ...
            switch (step) {
                ...
                case 3: {
                    ...
                    if (!stepData.interferenceAnalysis) {
                        throw new ValidationError(ErrorCode.MISSING_REQUIRED_FIELD, 'Step 3 (Interference Analysis) requires analyzing pattern interactions. ' +
                    ...
                    if (!analysis.constructive || !analysis.destructive) {
                        throw new ValidationError(ErrorCode.MISSING_REQUIRED_FIELD, 'Step 3 (Interference Analysis) requires BOTH constructive AND destructive interference patterns. ' +
                ...
                case 4:
                    ...
                    if (!stepData.computationalModels) {
                        throw new ValidationError(ErrorCode.MISSING_REQUIRED_FIELD, 'Step 4 (Computational Synthesis) requires synthesizing patterns using computational models. ' +
                ...
                case 5:
                    ...
                    if (!stepData.optimizationCycles || !stepData.convergenceMetrics) {
                        throw new ValidationError(ErrorCode.MISSING_REQUIRED_FIELD, 'Step 5 (Optimization) requires iterating to improve solution quality with measured progress. ' +
                            'Provide BOTH "optimizationCycles" (number) AND "convergenceMetrics" (object with coherence, novelty, utility). ' +
                            'Example: { "optimizationCycles": 10, "convergenceMetrics": { "coherence": 0.85, "novelty": 0.7, "utility": 0.9 }, "output": "..." }', 'optimizationCycles', {
                            step,
                            technique: 'neuro_computational',
                ...
                case 6:
                    ...
                    if (!stepData.finalSynthesis || !stepData.convergenceMetrics) {
                        throw new ValidationError(ErrorCode.MISSING_REQUIRED_FIELD, 'Step 6 (Convergence) requires achieving optimal solution with final metrics. ' +
                            'Provide BOTH "finalSynthesis" (string) AND "convergenceMetrics" (object with final scores). ' +
                            'Example: { "finalSynthesis": "Optimized solution achieving target performance", "convergenceMetrics": { "coherence": 0.95, "novelty": 0.8, "utility": 0.92 }, "output": "..." }', 'finalSynthesis', {
                            step,
                            technique: 'neuro_computational',
