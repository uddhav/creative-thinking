...
export async function executeThinkingStep(input, sessionManager, techniqueRegistry, visualFormatter, metricsCollector, complexityAnalyzer, ergodicityManager) {
    ...
    try {
        ...
        if (!handler.validateStep(input.currentStep, input)) {
            ...
            return responseBuilder.buildExecutionResponse(sessionId, operationData, [], nextStepGuidance, session.history.length);
        }
        ...
        if (input.pathImpact) {
            ergodicityResult = await ergodicityManager.recordThinkingStep(input.technique, input.currentStep, input.output, {
                ...
                commitmentLevel: input.pathImpact.commitmentLevel === 'low'
                    ? 0.2
        ...
        return responseBuilder.buildErrorResponse(error instanceof Error ? error : new Error('Unknown error'), 'execution');
    }
}
