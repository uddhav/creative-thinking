...
export class ExecutionValidator {
    ...
    calculateTechniqueLocalStep(input, plan) {
        ...
        if (input.planId && plan) {
            ...
            const containing = occurrences.find(o => input.currentStep > o.stepsBefore && input.currentStep <= o.stepsBefore + o.steps);
            ...
            if (input.numbering === 'technique') {
                ...
                if (input.currentStep < globalStartForTechnique) {
                    console.error(`Step ${input.currentStep} is before technique ${input.technique} which starts at global step ${globalStartForTechnique}`);
                    techniqueLocalStep = 1;
                    wasNormalized = true;
                }
                else {
                    console.error(`Step ${input.currentStep} is after technique ${input.technique} which ends at global step ${globalEndForTechnique}`);
            ...
            if (plan.workflow.length > 1 && currentTechniqueSteps > 0 && !wasNormalized) {
                const entryIsLocal = input.totalSteps === currentTechniqueSteps && input.totalSteps !== totalPlanSteps
                    ? true
                    : input.totalSteps === totalPlanSteps
                        ? false
    ...
    validateStepAndGetInfo(input, techniqueLocalStep, handler) {
        ...
        if (techniqueLocalStep < 1) {
            console.error(`Invalid technique-local step ${techniqueLocalStep} for ${input.technique}. Step must be >= 1. Using step 1 as fallback.`);
            techniqueLocalStep = 1;
            wasNormalized = true;
        }
        // Check if step is out of bounds
        if (techniqueLocalStep > maxSteps) {
            console.error(`Step ${techniqueLocalStep} exceeds maximum steps (${maxSteps}) for ${input.technique}. Using last step as fallback.`);
