...
export class ProblemAnalyzer {
     ...
     */
    categorizeProblemWithEvidence(problem, context) {
     ...
     */
    scoreCategories(nlpAnalysis, lowerText) {
        ...
        const countTerms = (terms, weight, cap) => Math.min(cap, terms.filter(t => lowerText.includes(t)).length * weight);
        const countNouns = (nouns, weight, cap) => Math.min(cap, nouns.filter(n => nounSet.has(n)).length * weight);
        const countVerbs = (verbs, weight, cap) => Math.min(cap, verbs.filter(v => verbSet.has(v)).length * weight);
        ...
        const paradoxical = nlpAnalysis.contradictions.hasContradiction &&
            !lowerText.includes('deadline') &&
            !lowerText.includes('conflicting requirements')
            ? 1.5
            : 0;
        const urgency = nlpAnalysis.temporal.urgency;
        const temporal = countTerms(['deadline', 'time management'], 2, 4) +
        ...
        const organizational = (entities.people.length > 2 ? 2 : 0) +
            ...
            countTerms(['team', 'restructure', 'reorgani', 'collaborat', 'department', 'headcount'], 1, 3);
        const validation = countTerms(['validation', 'validate', 'verify', 'hypothesis', 'evidence', 'authentic'], 1.5, 3) +
            ...
            (nlpAnalysis.pos.sentences.some(s => s.type === 'question' &&
                (s.text.toLowerCase().includes('true') ||
                    s.text.toLowerCase().includes('real') ||
                    s.text.toLowerCase().includes('valid')))
                ? 1
                : 0);
        const behavioral = countTerms(['customer behavior', 'user psychology'], 2, 2) +
        ...
        const fundamental = countTerms([
            ...
            (nlpAnalysis.pos.sentences.some(s => s.type === 'question' && s.text.toLowerCase().includes('why'))
                ? 1
                : 0);
        const learning = countTerms([
            'learn from',
            'synthesize pattern',
            'past failures',
            'past experience',
        ...
        const computational = countTerms([
        ...
        const cognitive = (this.COGNITIVE_PATTERN.test(lowerText) ? 2 : 0) +
            countVerbs(['focus', 'think', 'concentrate', 'remember'], 1, 2) +
            (topics.includes('psychology') ? 1 : 0);
        const implementationVerbs = countVerbs(['implement', 'execute', 'deploy', 'launch', 'build'], 1, 3);
        const implementation = implementationVerbs > 0
            ? implementationVerbs +
        ...
        const systemNouns = countNouns(['architecture', 'ecosystem'], 1.5, 3) + countNouns(['system', 'component'], 1, 2);
        const systems = systemNouns > 0 ? systemNouns + (topics.includes('technology') ? 1 : 0) : 0;
        const userCentered = countNouns(['user', 'customer', 'client'], 1, 2) +
        ...
        const technical = (nlpAnalysis.readability.clarity === 'very_complex'
            ? 1.5
        ...
        const creative = Math.min(2, entities.adjectives.filter(a => ['creative', 'innovative', 'novel', 'original'].includes(a.toLowerCase())).length) +
            countTerms(['fresh ideas', 'new ideas', 'brainstorm', 'out of the box', 'think outside'], 2, 2) +
        ...
        const process = processNouns > 0
            ? processNouns +
                Math.max(countVerbs(['optimize', 'improve', 'streamline'], 1, 2), countTerms(['optimiz', 'streamlin'], 1, 2))
            : 0;
        const strategic = (topics.includes('business') ? 1 : 0) +
            countNouns(['strategy', 'market', 'competition', 'competitor', 'pricing', 'revenue', 'positioning'], 1, 3) +
     ...
     */
    detectExplicitEndOfLife(lowerProblem) {
        ...
        if (/\b(retire|retiring|deprecate|deprecating|sunset|sunsetting)\s+(the|this|that|our|its|all)\b/.test(lowerProblem)) {
            return true;
        }
        ...
        const constructiveAsk = /\b(write|writing|draft|drafting|design|designing|build|building|create|creating|fix|fixing|improve|improving|redesign|rewrite)\b/.test(lowerProblem);
        if (constructiveAsk) {
            return false;
        }
        ...
        if (/\b(cancel|cancelling|canceling|renew|renewal|unsubscribe)\b/.test(lowerProblem) &&
     ...
     * earning its keep.
     */
    detectRetentionPattern(lowerText) {
     ...
     * means little else.
     */
    detectExplicitAdversarialAsk(lowerProblem) {
        ...
        const stressTestsAPosition = /\bstress[- ]test(ing|ed|s)?\b/.test(lowerProblem) &&
        ...
        const pokesHoles = /\b(poke[ds]?|poking|pick(ed|s)?|picking|punch(ed|es)?|punching|shoot|shot)\s+(some\s+|any\s+|a\s+few\s+)?holes\b/.test(lowerProblem);
        const convinceMeWrong = /\bconvince\s+(me|us)\s+(i'?m|we'?re|i am|we are)\s+wrong\b/.test(lowerProblem);
        ...
        const authoringVerb = /\b(write|writing|draft|drafting|document|documenting|author|authoring)\b/.test(lowerProblem);
        const documentNoun = /\b(template|templates|report|reports|playbook|policy|charter|checklist|guide|guideline|guidelines|curriculum|training|doc|docs|documentation|runbook|handbook)\b/.test(lowerProblem);
     ...
     */
    detectDecisionPattern(lowerText) {
     ...
     */
    detectCommunicationPattern(lowerText) {
     ...
     */
    detectCulturalPattern(lowerText) {
     ...
     */
    detectBiologicalPattern(lowerText) {
