...
export const ALL_HUMANISTIC_QUALITIES = [
    'intelligence',
    'courage',
    'tenacity',
    'curiosity',
 ...
 */
const TECHNIQUE_QUALITY_PROFILES = {
...
export class HumanisticQualityCoverage {
     ...
     */
    static getQualityProfile(technique) {
     ...
     */
    static analyzeCoverage(techniques) {
        const qualities = {
     ...
     */
    static fillCoverageGaps(recommendations) {
        // Don't modify small sets — not enough room to swap without losing value
        if (recommendations.length < 3) {
            const coverage = HumanisticQualityCoverage.analyzeCoverage(recommendations.map(r => r.technique));
        ...
        for (const gap of coverage.gaps) {
            if (fillersAdded >= MAX_FILLERS)
                break;
            const candidates = HumanisticQualityCoverage.findTechniquesForQuality(gap, currentRecs.map(r => r.technique));
        ...
        const finalCoverage = HumanisticQualityCoverage.analyzeCoverage(currentRecs.map(r => r.technique));
        return {
     ...
     */
    static findTechniquesForQuality(quality, excludeTechniques) {
...
//# sourceMappingURL=HumanisticQualityCoverage.js.map