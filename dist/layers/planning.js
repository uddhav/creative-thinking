...
export function planThinkingSession(input, sessionManager, techniqueRegistry) {
    ...
    if (strictness !== undefined && strictness !== 'advisory' && strictness !== 'enforcing') {
        planWarnings.push(`strictness "${strictness}" is not a recognized level ('advisory' now; 'enforcing' reserved) — the plan behaves as 'advisory'.`);
    }
    ...
    const integrationStrategy = techniques.length > 1 ? createIntegrationStrategy(techniques, timeframe) : undefined;
    ...
    if (lastRecommendations && lastRecommendations.length > 0) {
        ...
        techniques.forEach(selectedTechnique => {
            telemetry
                ...
                .catch(error => console.error('[Planning] Failed to track recommendation effectiveness:', error));
        });
    ...
    for (let i = 1; i <= totalSteps; i++) {
        ...
        if (technique === 'scamper' && 'getAction' in handler && 'getAllActions' in handler) {
            ...
            if (actionInfo) {
                ...
                const indicator = actionInfo.commitmentLevel === 'irreversible'
                    ? '🔒'
...
function getRequiredInputs(technique) {
    const inputs = {
        ...
        cultural_integration: ['Cultural contexts', 'Respect guidelines'],
...
function getExpectedOutputs(technique) {
    const outputs = {
...
function getIntegrationPoints(technique, allTechniques) {
    const points = [];
...
function createIntegrationStrategy(techniques, timeframe) {
...
function generateSuccessMetrics(objectives, timeframe) {
    const baseMetrics = [
...
function generateRiskMitigation(problem, constraints) {
    const mitigations = [];
...
function assessFlexibility(constraints) {
...
function getCriticalLensForStep(technique, step) {
...
function getRisksForStep(technique, step) {
 ...
 */
function generateTechniqueRationale(techniques, problem, constraints) {
    const techniqueReasons = {
        ...
        cultural_integration: 'culturally adaptive solutions',
 ...
 */
function generateSequenceLogic(techniques, integrationStrategy) {
    if (techniques.length === 1) {
    ...
    const hasGeneration = techniques.some(t => ['po', 'random_entry', 'scamper', 'yes_and'].includes(t));
    const hasIntegration = techniques.some(t => ['triz', 'collective_intel'].includes(t));
    const phases = [];
    if (hasUnderstanding)
        phases.push('understanding');
    if (hasGeneration)
...
function generateHistoricalNote(techniques, objectives) {
    ...
    const objectiveText = objectives && objectives.length > 0 ? ` for ${objectives[0].toLowerCase()}` : '';
 ...
 */
function assessComplexity(techniques, constraints, totalSteps) {
    ...
    if (score >= 4) {
        return {
            level: 'high',
            suggestion: 'This multi-phase plan could benefit from sequential thinking to manage dependencies and ensure systematic progress',
        };
    }
    else if (score >= 2) {
        return {
