...
export class ConvergenceValidator {
     ...
     */
    validateConvergence(input, options = {}) {
        ...
        const structureValidation = this.validateParallelResultsStructure(input.parallelResults, options);
     ...
     */
    validateParallelResultsExist(input) {
        ...
        if (!Array.isArray(input.parallelResults)) {
            const error = ErrorFactory.invalidInput('parallelResults', 'array', typeof input.parallelResults);
            return {
                isValid: false,
                error: this.errorHandler.handleError(error, 'execution', {
                    technique: input.technique,
     ...
     */
    validateParallelResultsStructure(parallelResults, options) {
     ...
     */
    validateResultObject(result, index) {
        if (!result || typeof result !== 'object' || Array.isArray(result)) {
            const error = ErrorFactory.invalidInput(`parallelResults[${index}]`, 'valid result object', result);
            return {
                isValid: false,
                error: this.errorHandler.handleError(error, 'execution', {
                    invalidIndex: index,
     ...
     */
    validateResultTechnique(result, index) {
        if (!result.technique || typeof result.technique !== 'string') {
            const error = ErrorFactory.missingParameter(`parallelResults[${index}].technique`, 'convergence');
            return {
                isValid: false,
                error: this.errorHandler.handleError(error, 'execution', {
                    invalidIndex: index,
        ...
        if (!validTechniques.includes(result.technique)) {
            const error = ErrorFactory.invalidInput(`parallelResults[${index}].technique`, `one of: ${validTechniques.join(', ')}`, result.technique);
            return {
                isValid: false,
                error: this.errorHandler.handleError(error, 'execution', {
                    invalidIndex: index,
     ...
     */
    validateResultInsights(result, index, options) {
        if (!Array.isArray(result.insights)) {
            const error = ErrorFactory.invalidInput(`parallelResults[${index}].insights`, 'array of strings', typeof result.insights);
            return {
                isValid: false,
                error: this.errorHandler.handleError(error, 'execution', {
                    invalidIndex: index,
        ...
        if (options.minInsights && result.insights.length < options.minInsights) {
            const error = ErrorFactory.invalidInput(`parallelResults[${index}].insights`, `at least ${options.minInsights} insights`, `${result.insights.length} insights`);
            return {
                isValid: false,
                error: this.errorHandler.handleError(error, 'execution', {
                    invalidIndex: index,
        ...
        if (options.maxInsights && result.insights.length > options.maxInsights) {
            const error = ErrorFactory.invalidInput(`parallelResults[${index}].insights`, `at most ${options.maxInsights} insights`, `${result.insights.length} insights`);
            return {
                isValid: false,
                error: this.errorHandler.handleError(error, 'execution', {
                    invalidIndex: index,
        ...
        for (let j = 0; j < result.insights.length; j++) {
            if (typeof result.insights[j] !== 'string') {
                const error = ErrorFactory.invalidInput(`parallelResults[${index}].insights[${j}]`, 'string', typeof result.insights[j]);
                return {
                    isValid: false,
                    error: this.errorHandler.handleError(error, 'execution', {
                        invalidIndex: index,
            ...
            if (result.insights[j].trim().length === 0) {
                const error = ErrorFactory.invalidInput(`parallelResults[${index}].insights[${j}]`, 'non-empty string', 'empty string');
                return {
                    isValid: false,
                    error: this.errorHandler.handleError(error, 'execution', {
                        invalidIndex: index,
     ...
     */
    validateOptionalFields(result, index, options) {
        // Validate results object
        if (options.requireResults && !result.results) {
            const error = ErrorFactory.missingParameter(`parallelResults[${index}].results`, 'convergence');
            return {
                isValid: false,
                error: this.errorHandler.handleError(error, 'execution', {
                    invalidIndex: index,
        ...
        if (result.results !== undefined && result.results !== null) {
            if (!this.isSerializableObject(result.results)) {
                const error = ErrorFactory.invalidInput(`parallelResults[${index}].results`, 'serializable plain object', typeof result.results);
                return {
                    isValid: false,
                    error: this.errorHandler.handleError(error, 'execution', {
                        invalidIndex: index,
        ...
        if (options.requireMetrics && !result.metrics) {
            const error = ErrorFactory.missingParameter(`parallelResults[${index}].metrics`, 'convergence');
            return {
                isValid: false,
                error: this.errorHandler.handleError(error, 'execution', {
                    invalidIndex: index,
        ...
        if (result.metrics !== undefined && result.metrics !== null) {
            if (!this.isSerializableObject(result.metrics)) {
                const error = ErrorFactory.invalidInput(`parallelResults[${index}].metrics`, 'serializable plain object', typeof result.metrics);
                return {
                    isValid: false,
                    error: this.errorHandler.handleError(error, 'execution', {
                        invalidIndex: index,
     ...
     * Validate convergence strategy
     */
    validateConvergenceStrategy(input) {
        ...
        if (!validStrategies.includes(input.convergenceStrategy)) {
            const error = ErrorFactory.invalidInput('convergenceStrategy', `one of: ${validStrategies.join(', ')}`, input.convergenceStrategy);
            return {
                isValid: false,
                error: this.errorHandler.handleError(error, 'execution', {
     ...
     */
    extractInsights(parallelResults) {
        const allInsights = [];
        for (const result of parallelResults) {
            if (result.insights && Array.isArray(result.insights)) {
                allInsights.push(...result.insights.filter(i => typeof i === 'string' && i.trim().length > 0));
            }
        }
        return allInsights;
    }
     ...
     */
    extractMetrics(parallelResults) {
        const metrics = [];
        for (const result of parallelResults) {
...
//# sourceMappingURL=ConvergenceValidator.js.map