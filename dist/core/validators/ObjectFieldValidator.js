...
export class ObjectFieldValidator {
     ...
     */
    static validateIsObject(value, fieldName) {
     ...
     */
    static handleStringValue(value, fieldName) {
        ...
        if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
            ...
            try {
                ...
                return {
                    isValid: false,
                    error: `${fieldName} contains invalid JSON: ${e.message}`,
     ...
     */
    static getExpectedFormat(fieldName) {
        const formats = {
            currentCell: '{ "timeFrame": "past|present|future", "systemLevel": "sub-system|system|super-system" }',
     ...
     */
    static validateCurrentCell(value) {
        ...
        if (!obj.systemLevel) {
            ...
            if (!['sub-system', 'system', 'super-system'].includes(systemLevelStr)) {
                errors.push(`Invalid systemLevel: "${systemLevelStr}". Must be one of: sub-system, system, super-system`);
            }
        }
        if (errors.length > 0) {
            return {
                ...
                suggestion: 'Provide currentCell as: { "timeFrame": "past|present|future", "systemLevel": "sub-system|system|super-system" }',
            };
        }
     ...
     */
    static validateNineWindowsMatrixItem(value, index) {
        ...
        const errors = [];
        // Required fields
        if (!obj.timeFrame || !['past', 'present', 'future'].includes(obj.timeFrame)) {
            errors.push('Invalid or missing timeFrame');
        }
        if (!obj.systemLevel ||
            !['sub-system', 'system', 'super-system'].includes(obj.systemLevel)) {
            ...
            if (typeof value === 'string') {
                ...
                if (trimmed.startsWith('[') && trimmed.endsWith(']')) {
                    try {
                        ...
                        return {
                            isValid: false,
                            error: `${fieldName} contains invalid JSON array string: ${e.message}`,
     ...
     */
    static validateDisneyMethodArrays(params) {
        // Check dreamerVision
     ...
     */
    static validateTechniqueArrayFields(technique, params) {
...
//# sourceMappingURL=ObjectFieldValidator.js.map