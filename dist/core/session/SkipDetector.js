...
export class SkipDetector {
    RUSH_THRESHOLD_SECONDS = 30; // Less than 30 seconds per step
    MIN_OUTPUT_LENGTH = 100; // Minimum characters for meaningful output
    CRITICAL_COMPLETION_THRESHOLD = 0.5; // 50% minimum for critical techniques
    /**
     * Analyze session for skip patterns
     */
    analyzeSession(session, plan) {
    ...
    analyzeUserPatterns(sessions) {
        ...
        sessionResults.forEach(result => {
            Object.entries(result.averageCompletionPerTechnique).forEach(([technique, completion]) => {
                ...
                techniqueStats.set(technique, stats);
            });
        });
        ...
        const totalSkipRate = sessionResults.reduce((sum, r) => sum + r.skipRate, 0) / sessionResults.length;
        // Determine trend (compare first half to second half)
        const midpoint = Math.floor(sessions.length / 2);
        const firstHalfRate = sessionResults.slice(0, midpoint).reduce((sum, r) => sum + r.skipRate, 0) / midpoint;
        const secondHalfRate = sessionResults.slice(midpoint).reduce((sum, r) => sum + r.skipRate, 0) /
     ...
     */
    calculateSessionStats(session, plan) {
        const techniqueCompletion = new Map();
        // Initialize from plan if available
        if (plan) {
            let stepIndex = 0;
            plan.workflow.forEach(workflow => {
                const completed = session.history.filter(h => h.technique === workflow.technique &&
                    h.currentStep > stepIndex &&
                    h.currentStep <= stepIndex + workflow.steps.length).length;
        ...
        // Calculate stats
        const totalSteps = plan?.totalSteps || session.history[session.history.length - 1]?.totalSteps || 0;
        const completedSteps = session.history.length;
        const skippedSteps = Math.max(0, totalSteps - completedSteps);
        const averageOutputLength = session.history.reduce((sum, h) => sum + h.output.length, 0) /
    ...
    detectPatterns(stats, session) {
        ...
        const avgTimePerStep = stats.timePerStep.reduce((a, b) => a + b, 0) / Math.max(1, stats.timePerStep.length);
        if (avgTimePerStep < this.RUSH_THRESHOLD_SECONDS &&
            stats.averageOutputLength < this.MIN_OUTPUT_LENGTH) {
            patterns.push('rush_through');
        }
        // Technique avoidance
        const avoidedTechniques = Array.from(stats.techniqueCompletion.entries()).filter(([_, data]) => data.total > 0 && data.completed === 0);
        if (avoidedTechniques.length > 0) {
            patterns.push('technique_avoidance');
        }
        // Critical step skip
        const hasBlackHatSkip = session.history.some(h => h.technique === 'six_hats' && h.currentStep === 5 && h.output.length < 50);
        if (hasBlackHatSkip) {
            patterns.push('critical_step_skip');
        }
        // Risk avoidance
        const riskSteps = session.history.filter(h => (h.technique === 'six_hats' && h.currentStep === 5) || // Black Hat
        ...
        if (session.history.length > 10) {
            ...
            const firstAvgLength = firstThird.reduce((sum, h) => sum + h.output.length, 0) / firstThird.length;
            const lastAvgLength = lastThird.reduce((sum, h) => sum + h.output.length, 0) / lastThird.length;
     ...
     */
    identifyCriticalSkips(session, _plan) {
        const criticalSkips = [];
     ...
     */
    generateRecommendations(patterns, criticalSkips, stats) {
        const recommendations = [];
        if (patterns.includes('early_termination')) {
            recommendations.push('Consider using a shorter workflow or breaking the problem into smaller parts');
        }
        ...
        if (patterns.includes('critical_step_skip') || criticalSkips.length > 0) {
            recommendations.push('Critical analysis steps were skipped - consider revisiting for comprehensive analysis');
        }
        if (patterns.includes('risk_avoidance')) {
            recommendations.push('Include risk assessment perspectives (Black Hat, TRIZ contradiction) for balanced analysis');
        }
        ...
        if (patterns.includes('selective_engagement')) {
            recommendations.push('Try to maintain consistent engagement across all techniques for balanced insights');
        }
     ...
     */
    calculateRiskScore(patterns, criticalSkips, stats) {
        let score = 0;
        // Pattern weights
     ...
     */
    calculateAverageCompletion(techniqueCompletion) {
        const result = {};
     ...
     */
    calculateEngagementVariance(techniqueCompletion) {
        ...
        const variance = completionRates.reduce((sum, rate) => sum + Math.pow(rate - mean, 2), 0) /
            completionRates.length;
        return Math.sqrt(variance);
    }
}
//# sourceMappingURL=SkipDetector.js.map