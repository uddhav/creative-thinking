...
function planFile(planId) {
    if (!PERSISTABLE_PLAN_ID.test(planId))
        return null;
 ...
 */
export function persistPlan(sessionManager, planId) {
    if (!planId || !plansArePersisted())
        return;
 ...
 */
export function hydratePlan(sessionManager, planId) {
    if (!planId || !plansArePersisted())
        return;
