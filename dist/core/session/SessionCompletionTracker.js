...
export class SessionCompletionTracker {
    ...
    calculateTechniqueStatuses(session, plan, isTerminating = false) {
        ...
        for (const workflow of plan.workflow) {
            ...
            if (repeats) {
                ...
                techniqueHistory = this.instanceHistory(pooled, index, globalStepOffset, techniqueSteps, plan.totalSteps);
            }
     ...
     */
    instanceHistory(pooled, index, globalStepOffset, techniqueSteps, planTotalSteps) {
        const usesPlanWide = planTotalSteps !== techniqueSteps &&
            pooled.some(entry => entry.totalSteps === planTotalSteps);
        if (usesPlanWide) {
    ...
    splitIntoInstances(history) {
        ...
        for (const entry of history) {
            ...
            const isRevision = entry.isRevision === true;
            const opensNewRun = current.length > 0 && !isRevision && seen.has(step) && step !== previousStep;
            if (opensNewRun) {
