...
export class MemoryAnalyzer {
     ...
     */
    generateMemoryOutputs(input, session) {
        const outputs = {};
    ...
    generateHistoricalNote(input, session) {
        ...
        const riskAwareness = session.history.filter(h => h.risks && h.risks.length > 0).length +
    ...
    identifyNoteworthyPattern(input, session) {
        ...
        const antifragileCount = session.history.reduce((sum, h) => sum + (h.antifragileProperties?.length || 0), 0);
