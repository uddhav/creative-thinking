...
export const PLAN_THINKING_SESSION_TOOL = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            strictness: {
                type: 'string',
                description: "Gate strictness for this plan's steps. 'advisory' (the default) attaches " +
...
export const EXECUTE_THINKING_STEP_TOOL = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            problem: {
                type: 'string',
                description: 'Taken from the plan named by planId when omitted; if sent, must match previous calls.',
            },
            numbering: {
                type: 'string',
                enum: ['technique', 'plan'],
                description: "OPTIONAL: which convention currentStep uses. 'technique' (default) counts within " +
            ...
            risks: {
                type: 'array',
                items: { type: 'string' },
                description: 'Risks identified at this step, usable with any technique. Counted in the session risk metrics together with failureModes, blackSwans, and the other technique-native risk fields.',
            },
            failureModes: {
                type: 'array',
                items: { type: 'string' },
                description: 'Ways the idea under examination can fail. Counted in the session risk metrics.',
            },
            mitigations: {
                type: 'array',
                items: { type: 'string' },
                description: 'Mitigations for risks already listed. Not counted as risks themselves — each one presumes a risk the other fields already carry.',
            },
            antifragileProperties: {
                type: 'array',
                items: { type: 'string' },
                description: 'Properties that make the idea gain from stress or disorder. Counted in the session antifragile metrics together with antifragileDesign and temporalEscapeRoutes.',
            },
            blackSwans: {
                type: 'array',
                items: { type: 'string' },
                description: 'Low-probability, high-impact events that would upend the idea. Counted in the session risk metrics.',
            },
            failureInsights: {
                type: 'array',
                items: { type: 'string' },
            ...
            stressTestResults: {
                type: 'array',
                items: { type: 'string' },
                description: 'Weaknesses surfaced by stress-testing the idea. Counted in the session risk metrics.',
            },
