...
export class PostgresAdapter {
    ...
    async initialize(config) {
        ...
        if (!connectionString) {
            throw new PersistenceError('PostgreSQL connection string not provided', PersistenceErrorCode.INVALID_FORMAT, { config });
        }
        ...
        try {
            ...
            throw new PersistenceError('Failed to connect to PostgreSQL', PersistenceErrorCode.IO_ERROR, error);
        }
    }
    /**
     * Create necessary tables if they don't exist
     */
    async createTablesIfNeeded(client) {
    ...
    `;
        ...
        try {
            ...
            throw new PersistenceError(`Failed to save session ${sessionId}`, PersistenceErrorCode.IO_ERROR, error);
        }
    }
    async load(sessionId) {
        ...
        try {
            ...
            throw new PersistenceError(`Failed to load session ${sessionId}`, PersistenceErrorCode.IO_ERROR, error);
        }
    }
    async delete(sessionId) {
        ...
        try {
            ...
            throw new PersistenceError(`Failed to delete session ${sessionId}`, PersistenceErrorCode.IO_ERROR, error);
        }
    }
    async exists(sessionId) {
        ...
        try {
            ...
            throw new PersistenceError(`Failed to check session existence ${sessionId}`, PersistenceErrorCode.IO_ERROR, error);
        }
    }
    async list(options = {}) {
        const pool = this.ensureInitialized();
    ...
    `;
        ...
        // Sorting
        const sortColumn = sortBy === 'created'
            ? 'created_at'
            : sortBy === 'updated'
                ? 'updated_at'
                : sortBy === 'name'
    ...
    async export(sessionId, format) {
        ...
        switch (format) {
            ...
            case 'markdown':
            case 'csv':
                throw new PersistenceError(`Export format ${format} not yet implemented`, PersistenceErrorCode.EXPORT_FAILED);
            default:
                throw new PersistenceError(`Unknown export format: ${format}`, PersistenceErrorCode.EXPORT_FAILED);
        }
    }
    async import(data, format) {
        if (format !== 'json') {
            throw new PersistenceError(`Import format ${format} not yet implemented`, PersistenceErrorCode.INVALID_FORMAT);
        }
        try {
            ...
            throw new PersistenceError('Failed to import session', PersistenceErrorCode.INVALID_FORMAT, error);
        }
    }
    async getStats() {
        const pool = this.ensureInitialized();
    ...
    `;
        try {
            ...
            throw new PersistenceError('Failed to cleanup old sessions', PersistenceErrorCode.IO_ERROR, error);
        }
    }
    ...
    `;
        try {
            ...
            throw new PersistenceError(`Failed to save plan ${planId}`, PersistenceErrorCode.IO_ERROR, error);
        }
    }
    async loadPlan(planId) {
        ...
        try {
            ...
            throw new PersistenceError(`Failed to load plan ${planId}`, PersistenceErrorCode.IO_ERROR, error);
        }
    }
    async deletePlan(planId) {
        ...
        try {
            ...
            throw new PersistenceError(`Failed to delete plan ${planId}`, PersistenceErrorCode.IO_ERROR, error);
        }
    }
    async close() {
        if (this.pool) {
