...
| 8   | Math-Elegance / Man-with-a-Hammer                        | Add models; never force one tool onto every problem                        | `mental_models` (whole technique), `nine_windows`, `first_principles`               | Economists' love of efficient-market elegance; "better to be roughly right than precisely wrong" — to the man with one hammer every problem is a nail. `mental_models` is the purpose-built latticework answer. |
...
● **The latticework of mental models** → `mental_models` (🧰). Munger's antidote to the one-hammer
trap is a multidisciplinary grid, and the `mental_models` technique is its direct implementation: 7
...
● **Latticework of mental models** → the `mental_models` technique. The canonical source text: grab
...
inside `mental_models` (physics/engineering lens), and is otherwise the natural territory of the
ergodicity subsystem and a Taleb-style fragility pass. ● **Margin of safety** — Ben Graham's
borrowing from engineering; it lands as `mental_models` step 7 and as a persona principle rather
than as a technique of its own.
...
the purpose-built `mental_models` latticework technique.
...
[`src/techniques/MentalModelsHandler.ts`](../src/techniques/MentalModelsHandler.ts). The 7-step
...
_Companion to the `cognitive_bias_audit` and `mental_models` techniques, the `charlie_munger`
persona, and the `munger-checklist` / `invert-problem` prompts. Sources: Charlie Munger, "The
Psychology of Human Misjudgment," Harvard, June 1995; and the 2008 DuBridge Distinguished Lecture,
Caltech._
