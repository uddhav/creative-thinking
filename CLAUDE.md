...
Provides 29 thinking techniques through a unified framework combining generative creativity with
...
│   ├── TechniqueRegistry.ts      # Singleton registry — imports and registers all 29 handlers
│   └── [TechniqueName]Handler.ts # One file per technique (30 files incl. GenericHandler fallback)
...
The `LateralTechnique` union type in `src/types/index.ts` defines all 29 valid technique
