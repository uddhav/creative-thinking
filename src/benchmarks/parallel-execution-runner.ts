...
function logHeader(message: string) {
  ...
  console.error(
    `${colors.bright}${colors.blue}${message.padStart((60 + message.length) / 2).padEnd(60)}${colors.reset}`
  );
...
): Promise<{ insights: string[]; duration: number; stepCount: number }> {
  ...
  for (let step = 1; step <= totalSteps; step++) {
    ...
    try {
      const response = await executeThinkingStep(
        ...
        dependencies.techniqueRegistry,
        dependencies.visualFormatter,
        dependencies.metricsCollector,
        dependencies.complexityAnalyzer,
        dependencies.ergodicityManager
      );
...
async function runBasicComparison(deps: BenchmarkDeps) {
  ...
  for (const technique of techniques) {
    ...
    log(
      `  ✓ ${technique}: ${result.duration.toFixed(0)}ms (${result.stepCount} steps, ${result.insights.length} insights)`,
      colors.green
    );
  }
  ...
  const parPromises = techniques.map(technique =>
    executeTechnique(technique, problem, plan.planId, deps, {
      simulateDelay: 50,
      verbose: true,
    })
  );
...
async function runScalabilityTest(deps: BenchmarkDeps) {
  ...
  for (let count = 2; count <= 6; count++) {
    ...
    const parStart = performance.now();
    await Promise.all(
      techniques.map(t => executeTechnique(t, problem, plan.planId, deps, { simulateDelay: 30 }))
    );
    ...
    log(
      `  Sequential: ${seqTime.toFixed(0)}ms | Parallel: ${parTime.toFixed(0)}ms | Speedup: ${colors.green}${speedup.toFixed(2)}x${colors.reset}\n`
    );
  }
...
async function runRealWorldScenario(deps: BenchmarkDeps) {
  ...
  const techniques: LateralTechnique[] = ['six_hats', 'design_thinking', 'triz', 'scamper'];
  ...
  try {
    await executeThinkingStep(
      ...
      deps.visualFormatter,
      deps.metricsCollector,
      deps.complexityAnalyzer,
      deps.ergodicityManager
    );
    ...
    log(
      `Overall Speedup: ${colors.green}${(theoreticalSeqTime / totalTime).toFixed(2)}x${colors.reset}`
    );
