...
export class PrivacyManager {
  private sessionHashMap = new Map<string, string>();
  private salt: string;
   ...
   */
  private sanitizeMetrics(metadata: TelemetryEvent['metadata']): PrivacySafeEvent['metrics'] {
    const metrics: PrivacySafeEvent['metrics'] = {};
   ...
   */
  private shouldExclude(event: TelemetryEvent): boolean {
   ...
   */
  private fuzzyTimestamp(timestamp: number): number {
    if (this.config.privacyMode === 'strict') {
