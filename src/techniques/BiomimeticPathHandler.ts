...
export class BiomimeticPathHandler extends BaseTechniqueHandler {
      ...
      switch (step) {
        case 1:
          ...
          if (!stepData.immuneResponse && !stepData.antibodies) {
            throw new ValidationError(
              ...
              'Step 1 (Immune Response Detection) requires identifying defensive biological patterns. ' +
                'Provide either "immuneResponse" (array of threat responses) or "antibodies" (array of defense mechanisms). ' +
                'Example: { "immuneResponse": ["Pattern recognition", "Adaptive immunity", "Memory formation"], "output": "..." }',
              'immuneResponse',
              {
                step,
        ...
        case 2:
          ...
          if (!stepData.mutations && !stepData.selectionPressure) {
            throw new ValidationError(
              ErrorCode.MISSING_REQUIRED_FIELD,
              'Step 2 (Evolutionary Variation) requires generating solution mutations and selection criteria. ' +
                'Provide either "mutations" (array of variations) or "selectionPressure" (string describing criteria). ' +
                'Example: { "mutations": ["variant A", "variant B", "variant C"], "output": "..." }',
              'mutations',
              {
                step,
                technique: 'biomimetic_path',
        ...
        case 3:
          ...
          if (!stepData.symbioticRelationships && !stepData.ecosystemBalance) {
            throw new ValidationError(
              ErrorCode.MISSING_REQUIRED_FIELD,
              'Step 3 (Ecosystem Dynamics) requires mapping symbiotic relationships and resource flows. ' +
                'Provide either "symbioticRelationships" (array) or "ecosystemBalance" (string). ' +
                'Example: { "symbioticRelationships": ["mutualism between A and B", "commensalism with C"], "output": "..." }',
        ...
        case 4:
          ...
          if (!stepData.swarmBehavior && !stepData.emergentPatterns) {
            throw new ValidationError(
              ErrorCode.MISSING_REQUIRED_FIELD,
              'Step 4 (Swarm Intelligence) requires designing collective behavior patterns. ' +
                'Provide either "swarmBehavior" (array) or "emergentPatterns" (array). ' +
                'Example: { "swarmBehavior": ["simple rule 1", "local interaction 2", "collective decision 3"], "output": "..." }',
              'swarmBehavior',
              {
                step,
                technique: 'biomimetic_path',
        ...
        case 5:
          ...
          if (!stepData.resiliencePatterns && !stepData.redundancy) {
            throw new ValidationError(
              ErrorCode.MISSING_REQUIRED_FIELD,
              'Step 5 (Resilience Patterns) requires building antifragile systems. ' +
                'Provide either "resiliencePatterns" (array) or "redundancy" (array). ' +
                'Example: { "resiliencePatterns": ["modular design", "adaptive cycles", "fault tolerance"], "output": "..." }',
              'resiliencePatterns',
              {
                step,
                technique: 'biomimetic_path',
        ...
        case 6:
          ...
          if (
            !stepData.naturalSynthesis &&
            !stepData.integratedSolution &&
            !stepData.biologicalStrategies
          ) {
            throw new ValidationError(
              ErrorCode.MISSING_REQUIRED_FIELD,
              'Step 6 (Natural Synthesis) requires integrating biological solutions into a unified approach. ' +
                'Provide "naturalSynthesis" (string), "integratedSolution" (string), or "biologicalStrategies" (array). ' +
                'Example: { "naturalSynthesis": "Integrated bio-inspired solution combining immune response with swarm intelligence", "output": "..." }',
              'naturalSynthesis',
              {
                step,
                technique: 'biomimetic_path',
   ...
   */
  private renderAlias(entry: Record<string, unknown>, ...names: string[]): string {
