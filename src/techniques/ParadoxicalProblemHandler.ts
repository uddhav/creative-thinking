...
export class ParadoxicalProblemHandler extends BaseTechniqueHandler {
  ...
  getTechniqueInfo(): TechniqueInfo {
    return {
      ...
      totalSteps: 4,
      description:
        'PRESERVE contradictions by building infrastructure to manage both sides dynamically',
      ...
      switch (step) {
        case 1:
          ...
          if (!stepData.paradox && !stepData.contradictions && !stepData.contradiction) {
            throw new ValidationError(
              ...
              'Step 1 (Paradox Identification) requires identifying contradictory requirements. ' +
                'Provide "paradox" (string), "contradiction" (string), or "contradictions" (array). ' +
                'Example: { "paradox": "Need both speed and accuracy which conflict", "output": "..." }',
              'paradox',
              {
                step,
        ...
        case 2:
          ...
          if (!stepData.solutionA && !stepData.solutionB && !stepData.parallelPaths) {
            throw new ValidationError(
              ErrorCode.MISSING_REQUIRED_FIELD,
              'Step 2 (Parallel Path Development) requires developing independent solutions. ' +
                'Provide "solutionA" (string), "solutionB" (string), or "parallelPaths" (array). ' +
                'Example: { "solutionA": "Optimize for speed", "solutionB": "Optimize for accuracy", "output": "..." }',
              'parallelPaths',
              {
                step,
                technique: 'paradoxical_problem',
        ...
        case 3:
          ...
          if (!stepData.synthesis && !stepData.metaPath && !stepData.bridge) {
            throw new ValidationError(
              ErrorCode.MISSING_REQUIRED_FIELD,
              'Step 3 (Transcendent Synthesis) requires creating a meta-solution. ' +
                'Provide "synthesis" (string), "metaPath" (string), or "bridge" (string). ' +
                'Example: { "synthesis": "Dynamic switching between solutions based on context", "output": "..." }',
              'synthesis',
              {
                step,
                technique: 'paradoxical_problem',
        ...
        case 4:
          ...
          if (
            ...
            throw new ValidationError(
              ErrorCode.MISSING_REQUIRED_FIELD,
              'Step 4 (Non-Ergodic Validation) requires validating the resolution. ' +
                'Provide "validation" (string), "pathContexts" (array), "resolutionVerified" (boolean), or "finalSynthesis" (string). ' +
                'Example: { "validation": "Resolution tested across multiple contexts", "output": "..." }',
              'validation',
              {
                step,
                technique: 'paradoxical_problem',
