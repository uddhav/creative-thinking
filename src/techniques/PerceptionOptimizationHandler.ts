...
interface PerceptionGap {
  objective: string;
  perceived: string;
  gapSize: 'small' | 'medium' | 'large' | 'massive';
  leverageOpportunity: 'low' | 'medium' | 'high' | 'very_high';
}

interface PerceptionOptimizationStep extends StepInfo {
  elements?: string[];
  optimizationType?: string;
}

export class PerceptionOptimizationHandler extends BaseTechniqueHandler {
  private readonly steps: PerceptionOptimizationStep[] = [
    {
  ...
  getTechniqueInfo(): TechniqueInfo {
    return {
      ...
      enhancedFocus:
        'Systematically optimizing subjective experience to create outsized perceived value',
      parallelSteps: {
  ...
  getStepInfo(step: number): StepInfo {
    if (step < 1 || step > this.steps.length) {
      throw new ValidationError(
        ...
        `Invalid step ${step} for Perception Optimization. Valid steps are 1-${this.steps.length}`,
        'step',
  ...
  getStepGuidance(step: number, problem: string): string {
    const stepInfo = this.steps[step - 1] as PerceptionOptimizationStep | undefined;
...
Output: Complete activation plan with perception metrics and success criteria`,
    ...
    return (
      guidanceTemplates[step] || `Complete the Perception Optimization process for: "${problem}"`
    );
  }
  ...
  validateStep(step: number, data: unknown): boolean {
    ...
    if (step === 1 && data && typeof data === 'object' && 'perceptionGaps' in data) {
      ...
      return gaps.every((gap: unknown) => {
        if (typeof gap !== 'object' || !gap) return false;
        const gapObj = gap as Record<string, unknown>;
        const validSize = ['small', 'medium', 'large', 'massive'].includes(
          gapObj.gapSize as string
        );
        const validLeverage = ['low', 'medium', 'high', 'very_high'].includes(
          gapObj.leverageOpportunity as string
        );
        return (
          typeof gapObj.objective === 'string' &&
