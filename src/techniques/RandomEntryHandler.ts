...
How can the forced connections become real innovations?`;
  ...
  extractInsights(
    ...
    history.forEach(entry => {
      ...
      if (entry.currentStep === 3 && entry.output) {
        ...
        if (ideas && ideas.length > 0) {
          if (isRoryMode) {
            insights.push(
              `Generated ${ideas.length} counterintuitive solutions using behavioral insights`
            );
          } else {
    ...
    if (isRoryMode && history.length >= 3) {
      insights.push(
        'Rory Mode: Applied behavioral economics principles to generate non-obvious solutions'
      );
    }
