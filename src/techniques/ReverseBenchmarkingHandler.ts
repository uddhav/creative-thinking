...
Output: Complete implementation plan with timeline and success metrics`,
  ...
  extractInsights(history: HistoryEntry[]): string[] {
    ...
    const latestByStep = new Map<number, HistoryEntry>();
    ...
    for (let step = 1; step <= totalSteps; step++) {
      ...
      if (step === 1) {
        ...
        if (weaknesses.length > 0) {
          insights.push(
            `${stepName}: Identified ${weaknesses.length} universal competitor weaknesses — ${weaknesses.join(', ')}`
          );
        }
      }
      if (step === 2 && Array.isArray(entry.vacantSpaces)) {
        const spaces = entry.vacantSpaces;
        const highValue = spaces.filter(
        ...
        if (highValue.length > 0) {
          ...
          insights.push(
            `${stepName}: ${highValue.length} of ${spaces.length} vacant spaces rated high or very high — ${described}.${remainder}`
          );
        }
      }
      if (step === 3) {
        ...
        const strategy =
          typeof entry.antiMimeticStrategy === 'string'
