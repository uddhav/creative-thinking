...
export class CulturalCreativityHandler extends BaseTechniqueHandler {
  private readonly steps: CulturalCreativityStep[] = [
    {
      name: 'Cultural Mapping',
      focus: 'Map cultural contexts and frameworks',
      emoji: '🗺️',
      description:
        'Understand historical contexts, recognize power dynamics, map cognitive frameworks, and document cultural constraints or taboos',
    },
    {
      ...
      emoji: '🤝',
      description:
        'Discover natural connections, shared experiences, complementary strengths, and potential friction zones between cultures',
    },
    {
      ...
      emoji: '🌉',
      description:
        'Develop translation protocols, design bidirectional exchange mechanisms, and build trust through authentic engagement',
    },
    {
      ...
      emoji: '🎨',
      description:
        'Synthesize innovations that acknowledge all sources, avoid superficial adoption, and maintain cultural authenticity',
    },
  ];
  ...
  getTechniqueInfo(): TechniqueInfo {
    return {
      ...
      totalSteps: 4,
      description:
        'Orchestrate multi-cultural creative synthesis without appropriation through respectful integration and proper attribution',
      focus: 'Respectful multi-cultural innovation synthesis',
      enhancedFocus:
        'Creates culturally-aware innovations through mapping, touchpoint identification, bridge building, and authentic synthesis with full attribution',
      parallelSteps: {
  ...
  getStepInfo(step: number): CulturalCreativityStep {
    ...
    if (!stepInfo) {
      throw new ValidationError(
        ...
        `Invalid step ${step} for Cultural Creativity Orchestration. Valid steps are 1-${this.steps.length}`,
        'step',
   ...
   */
  private isValidStepData(data: unknown): data is Record<string, unknown> {
   ...
   */
  private validateCulturalMapping(stepData: Record<string, unknown>): void {
    if (!stepData.culturalContexts || !stepData.powerDynamics) {
      throw new ValidationError(
        ErrorCode.MISSING_REQUIRED_FIELD,
        'Step 1 requires both cultural contexts AND power dynamics analysis. ' +
          'Expected format: { culturalContexts: string[], powerDynamics: { [culture: string]: string } }',
        'culturalMapping',
        {
          step: 1,
   ...
   */
  private validateTouchpointIdentification(stepData: Record<string, unknown>): void {
    if (!stepData.naturalConnections || !stepData.frictionZones) {
      throw new ValidationError(
        ErrorCode.MISSING_REQUIRED_FIELD,
        'Step 2 requires both natural connections AND friction zones identification. ' +
          'Expected format: { naturalConnections: string[], frictionZones: string[] }',
   ...
   */
  private validateBridgeBuilding(stepData: Record<string, unknown>): void {
    if (!stepData.translationProtocols || !stepData.trustMechanisms) {
      throw new ValidationError(
        ErrorCode.MISSING_REQUIRED_FIELD,
        'Step 3 requires both translation protocols AND trust mechanisms. ' +
          'Expected format: { translationProtocols: object, trustMechanisms: string[] }',
        'bridgeBuilding',
        {
          step: 3,
   ...
   */
  private validateAuthenticSynthesis(stepData: Record<string, unknown>): void {
    if (!stepData.attributionMap || !stepData.authenticityMeasures) {
      throw new ValidationError(
        ErrorCode.MISSING_REQUIRED_FIELD,
        'Step 4 requires both attribution map AND authenticity measures. ' +
          'Expected format: { attributionMap: object, authenticityMeasures: string[] }',
        'synthesis',
        {
          step: 4,
