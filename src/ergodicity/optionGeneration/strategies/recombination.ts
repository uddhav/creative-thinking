...
export class RecombinationStrategy extends BaseOptionStrategy {
  readonly strategyName = 'recombination' as const;
  readonly description = 'Generate new options by recombining existing elements in novel ways';
  readonly typicalFlexibilityGain = { min: 0.15, max: 0.3 };
  readonly applicableCategories: OptionCategory[] = ['structural', 'process', 'technical'];
  ...
  isApplicable(context: OptionGenerationContext): boolean {
    ...
    const hasVariedDecisions =
      new Set(context.pathMemory.pathHistory.map(e => e.technique)).size > 2;
  ...
  estimateEffort(option: Option): 'low' | 'medium' | 'high' {
    if (option.name.includes('Feature')) return 'low';
    if (option.name.includes('Cross') || option.name.includes('Hybrid')) return 'medium';
    return 'high'; // Synthesis requires more integration work
  }

  private extractRecombinableElements(context: OptionGenerationContext): {
    successfulPatterns: string[];
    availableComponents: string[];
    complementaryPairs: [string, string][];
    unusedCombinations: string[];
  } {
    const analysis = {
      successfulPatterns: [] as string[],
      availableComponents: [] as string[],
      complementaryPairs: [] as [string, string][],
      unusedCombinations: [] as string[],
    };
  ...
  private createCrossPollinationOption(
    elements: ReturnType<typeof this.extractRecombinableElements>,
    context: OptionGenerationContext
  ): Option | null {
    if (elements.complementaryPairs.length === 0) return null;

    const [technique1, technique2] = elements.complementaryPairs[0];

    // Check if this combination has been tried before
    const previousAttempts = context.pathMemory.pathHistory.filter(
      e => e.decision.includes(technique1) && e.decision.includes(technique2)
    ).length;
    ...
    return this.createOption(
      `Cross-Pollinate ${this.formatTechniqueName(technique1)} with ${this.formatTechniqueName(technique2)}`,
      `Combine insights from ${technique1} and ${technique2} approaches. Their complementary strengths can create solutions neither could achieve alone.${noveltyNote}`,
      'process',
      actions,
      ['Review of both technique outputs', 'Open mindset for integration']
    );
  }
  ...
  private createHybridOption(
    ...
    const hasTeamConstraints = context.pathMemory.constraints.some(
      c => c.type === 'relational' || c.description.toLowerCase().includes('team')
    );
    ...
    return this.createOption(
      ...
      `Merge ${pattern1} with ${pattern2} to create a hybrid approach. This combines proven elements in new ways to address current constraints.${teamNote}`,
      'structural',
      actions,
      hasTeamConstraints
  ...
  private createFeatureMigrationOption(
    ...
    const previousMigrations = context.pathMemory.pathHistory.filter(
      e =>
        e.decision.toLowerCase().includes('migrate') ||
        e.decision.toLowerCase().includes('transfer')
    ).length;
    ...
    const migrationStrategy =
      previousMigrations > 1 ? 'selective' : isLowFlexibility ? 'essential' : 'comprehensive';
    ...
    return this.createOption(
      'Migrate Successful Features',
      `Transfer successful patterns from ${sourceComponent} to ${targetComponent}. Proven solutions in one area often work in others with adaptation.${strategyNote}`,
      'technical',
      actions,
      prerequisites
    );
  ...
  private createSynthesisOption(
    ...
    const pastSynthesisAttempts = context.pathMemory.pathHistory.filter(
      e =>
        e.decision.toLowerCase().includes('synthesize') ||
        e.decision.toLowerCase().includes('novel')
    ).length;

    const isHighFlexibility = context.currentFlexibility.flexibilityScore > 0.6;
    const innovationLevel =
      pastSynthesisAttempts > 2 ? 'radical' : isHighFlexibility ? 'bold' : 'measured';
    ...
    return this.createOption(
      ...
      `Create novel solution by synthesizing ${unusedCombo}. Unexplored combinations often yield breakthrough insights.${innovationNote}`,
      'structural',
      actions,
      prerequisites
    );
  ...
  private extractPattern(decision: string): string | null {
    const patterns = [
      'incremental',
      'iterative',
      'parallel',
    ...
    if (found) return found + ' approach';

    // Extract action patterns
    if (lowerDecision.includes('test')) return 'test-driven approach';
    if (lowerDecision.includes('prototype')) return 'prototyping approach';
    if (lowerDecision.includes('analyze')) return 'analytical approach';

    return null;
  }

  private extractComponents(decision: string): string[] {
  ...
  private areComplementary(technique1: string, technique2: string): boolean {
    ...
    return complementaryPairs.some(
      ([a, b]) => (technique1 === a && technique2 === b) || (technique1 === b && technique2 === a)
    );
  }

  private findUnusedCombinations(components: string[]): string[] {
