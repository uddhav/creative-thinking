...
export class ResourceStrategy extends BaseOptionStrategy {
  readonly strategyName = 'resource' as const;
  ...
  readonly applicableCategories: OptionCategory[] = ['resource', 'process'];
  ...
  estimateEffort(option: Option): 'low' | 'medium' | 'high' {
    if (option.name.includes('Reallocate')) return 'low';
    if (option.name.includes('Share') || option.name.includes('Substitute')) return 'medium';
    return 'high'; // Multiplication strategies require more setup
  }

  private analyzeResourceAllocation(context: OptionGenerationContext): {
    ...
    const resourceMentions = new Map<string, number>();
  ...
  private createReallocationOption(
    resources: ReturnType<typeof this.analyzeResourceAllocation>,
    ...
    const pastReallocations = context.pathMemory.pathHistory.filter(
      e =>
        e.decision.toLowerCase().includes('reallocate') ||
        e.decision.toLowerCase().includes('shift')
    ).length;
    ...
    return this.createOption(
      `Reallocate ${this.formatResourceName(fromResource)}`,
      `Shift ${fromResource} from over-allocated areas to ${toResource}. Current allocation shows imbalance that limits flexibility. Rebalancing creates room for new initiatives.${urgencyNote}`,
      'resource',
      actions,
      prerequisites
    );
  ...
  private createResourceSharingOption(
    ...
    const sharableResource =
      resources.sharable.find(r => resources.bottlenecks.includes(r)) || resources.sharable[0];
    ...
    const hasCollaborationConstraints = context.pathMemory.constraints.some(
      c => c.type === 'relational' || c.description.toLowerCase().includes('partner')
    );
    const pastSharingAttempts = context.pathMemory.pathHistory.filter(
      e => e.decision.toLowerCase().includes('share') || e.decision.toLowerCase().includes('pool')
    ).length;
    ...
    return this.createOption(
      `Create ${this.formatResourceName(sharableResource)} Sharing Pool`,
      `Transform individual ${sharableResource} ownership into shared pool. This multiplies effective capacity without additional investment and creates collaborative benefits.${modelNote}`,
      'resource',
      actions,
      prerequisites
    );
  ...
  private createSubstitutionOption(
    ...
    const pastSubstitutions = context.pathMemory.pathHistory.filter(
      e =>
        e.decision.toLowerCase().includes('substitute') ||
        e.decision.toLowerCase().includes('replace')
    ).length;
    const isHighPressure = context.currentFlexibility.flexibilityScore < 0.3;

    // Adjust substitution approach based on context
    const approach =
      pastSubstitutions > 2 ? 'innovative' : isHighPressure ? 'pragmatic' : 'systematic';
    ...
    return this.createOption(
      `Substitute ${this.formatResourceName(bottleneckResource)} with ${substitute}`,
      `Replace scarce ${bottleneckResource} with abundant ${substitute}. This breaks dependency on constrained resources and opens new possibilities.${approachNote}`,
      'resource',
      actions,
      prerequisites
    );
  ...
  private createMultiplicationOption(
    ...
    const targetResource =
      resources.bottlenecks[0] || resources.overAllocated[0] || 'key resources';

    // Check for technical capabilities and past automation attempts
    const hasTechnicalConstraints = context.pathMemory.constraints.some(
      c => c.type === 'technical' || c.description.toLowerCase().includes('technical')
    );
    const pastAutomationAttempts = context.pathMemory.pathHistory.filter(
      e =>
        e.decision.toLowerCase().includes('automate') ||
        e.decision.toLowerCase().includes('template')
    ).length;
    ...
    return this.createOption(
      'Multiply Resource Impact',
      `Transform one-time ${targetResource} use into reusable assets. Create templates, ${multiplicationStrategy === 'manual' ? 'processes' : 'automation'}, and network effects that multiply the impact of every resource unit invested.${strategyNote}`,
      'resource',
      actions,
      prerequisites
    );
  ...
  private extractResourceType(description: string): string {
    ...
    if (lowerDesc.includes('time')) return 'time';
    if (lowerDesc.includes('budget') || lowerDesc.includes('money')) return 'budget';
    if (lowerDesc.includes('people') || lowerDesc.includes('team')) return 'people';
    if (lowerDesc.includes('attention') || lowerDesc.includes('focus')) return 'attention';
    if (lowerDesc.includes('expertise') || lowerDesc.includes('skill')) return 'expertise';
    if (lowerDesc.includes('tool') || lowerDesc.includes('system')) return 'tools';

    return 'resources';
  }

  private extractResourceMentions(text: string): string[] {
  ...
  private findSubstitute(resource: string): string {
  ...
  private createGeneralReallocationOption(): Option {
    return this.createOption(
      'Optimize Resource Allocation',
      'Conduct comprehensive resource audit to identify misallocations. Even small shifts from low-value to high-value activities can significantly increase flexibility.',
      'resource',
      [
