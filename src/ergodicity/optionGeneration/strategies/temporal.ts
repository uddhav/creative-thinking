...
export class TemporalStrategy extends BaseOptionStrategy {
  ...
  readonly description =
    'Create flexibility by changing time parameters - delay, accelerate, or sequence differently';
  readonly typicalFlexibilityGain = { min: 0.1, max: 0.3 };
  readonly applicableCategories: OptionCategory[] = ['temporal', 'process'];
  ...
  estimateEffort(option: Option): 'low' | 'medium' | 'high' {
    ...
    if (option.name.includes('Delay')) return 'low';
    if (option.name.includes('Accelerate')) return 'medium';
    return 'medium'; // Reordering
  }

  private identifyTemporalOpportunities(context: OptionGenerationContext): TemporalOpportunity[] {
    const opportunities: TemporalOpportunity[] = [];
  ...
  private createDelayOption(
    opportunity: TemporalOpportunity,
    ...
    return this.createOption(
      `Delay ${this.extractDecisionName(opportunity.description)}`,
      `Postpone ${opportunity.description} for ${delayPeriod}. ${opportunity.delayBenefit}. This preserves flexibility to adapt as new information emerges.`,
      'temporal',
      actions,
      prerequisites
    );
  ...
  private createAccelerationOption(
    ...
    const pastAccelerations = context.pathMemory.pathHistory.filter(
      e =>
        e.decision.toLowerCase().includes('accelerate') ||
        e.decision.toLowerCase().includes('fast-track')
    ).length;
    ...
    const accelerationMode =
      pastAccelerations > 2 ? 'selective' : hasResourceConstraints ? 'lean' : 'intensive';
    ...
    return this.createOption(
      `Accelerate ${this.extractDecisionName(opportunity.description)}`,
      `Fast-track ${opportunity.description} to capture time-sensitive value. ${opportunity.accelerationBenefit}. Rapid execution prevents option expiry.${modeNote}`,
      'temporal',
      actions,
      prerequisites
    );
  ...
  private createReorderingOption(context: OptionGenerationContext): Option | null {
    ...
    return this.createOption(
      'Resequence Process Steps',
      `Reorder process steps to increase flexibility. ${dependencies.reorderablePairs.length} pairs of steps can be reordered without breaking dependencies. This creates options for parallel execution and faster adaptation.`,
      'process',
      actions,
      ['Dependency analysis complete', 'Team briefed on changes']
    );
  }
  ...
  private extractDecisionName(description: string): string {
    ...
    const verbIndex = words.findIndex(word =>
      actionVerbs.some(verb => word.toLowerCase().includes(verb))
    );
  ...
  private hasReorderingPotential(context: OptionGenerationContext): boolean {
    ...
    const hasSequentialPattern = recentSteps.every(
      (step, index) => index === 0 || step.timestamp > recentSteps[index - 1].timestamp
    );

    return hasSequentialPattern;
  }

  private analyzeDependencies(context: OptionGenerationContext): {
    reorderablePairs: [string, string][];
  } {
    const reorderablePairs: [string, string][] = [];
    ...
    for (let i = 0; i < recentSteps.length - 1; i++) {
      for (let j = i + 1; j < recentSteps.length; j++) {
        ...
        if (step1.commitmentLevel < 0.5 && step2.commitmentLevel < 0.5) {
          ...
          const step1ClosesOptionsForStep2 = step1.optionsClosed.some(opt =>
            step2.decision.toLowerCase().includes(opt.toLowerCase())
          );
