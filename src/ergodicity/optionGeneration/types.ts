...
export interface Option {
  ...
  // Meta information
  generatedAt: string;
  expiresAt?: string; // Some options are time-sensitive
...
export type OptionCategory =
  | 'structural' // Changes to system structure
  | 'temporal' // Time-based modifications
  | 'relational' // Stakeholder/relationship changes
  | 'resource' // Resource allocation changes
  | 'capability' // Skill/capability development
  | 'conceptual' // Abstract/conceptual shifts
  | 'process' // Process modifications
  | 'technical'; // Technical solutions

/**
 * Generation strategies available
 */
export type OptionGenerationStrategy =
  | 'decomposition'
  | 'recombination'
  | 'abstraction'
  | 'inversion'
...
export interface OptionEvaluation {
  optionId: string;
  flexibilityGain: number; // 0.0-1.0: How much flexibility this adds
  implementationCost: number; // 0.0-1.0: Resources required (0=low, 1=high)
  reversibility: number; // 0.0-1.0: How easy to undo (0=hard, 1=easy)
  synergyScore: number; // 0.0-1.0: Compatibility with other options
  timeToValue: number; // Days until benefits appear
  overallScore: number; // Composite score for ranking
...
export interface PathMemory {
  ...
  flexibilityOverTime: Array<{ step: number; score: number; timestamp: number }>;
  barrierProximity?: Array<{ barrier: Barrier; distance: number }>;
...
export interface OptionGenerationResult {
  ...
  context: {
    ...
    projectedFlexibility: number; // If all options implemented
