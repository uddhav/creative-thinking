...
export class MetricsCalculator {
   ...
   */
  private calculateOptionVelocity(pathMemory: PathMemory): number | undefined {
    ...
    const measured = recentEvents.some(
      event => event.optionsOpened.length > 0 || event.optionsClosed.length > 0
    );
  ...
  getMetricsSummary(metrics: FlexibilityMetrics): string {
    const lines: string[] = [
      ...
      `├─ Option Velocity: ${
        metrics.optionVelocity === undefined
          ? 'unmeasured'
