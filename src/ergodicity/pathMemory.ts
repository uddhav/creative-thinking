...
export class PathMemoryManager {
   ...
   */
  static computeFlexibilityScore(events: ReadonlyArray<{ flexibilityImpact?: number }>): number {
  ...
  private updateFlexibilityMetrics(): void {
    ...
    this.pathMemory.currentFlexibility.flexibilityScore = PathMemoryManager.computeFlexibilityScore(
  ...
  private calculateBarrierProximity(barrier: Barrier): number {
    ...
    switch (barrier.subtype) {
      case 'cognitive_lock_in': {
        ...
        const irreversible = recent.filter(
          e => e.reversibilityCost > LOW_REVERSIBILITY_COST
        ).length;
