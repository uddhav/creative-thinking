...
export class LateralThinkingServer {
  ...
  private generateHistoricalNote(
    ...
    if (stepCount >= 3) {
      ...
      const riskMentions = session.history.filter(h => 
  ...
  private assessBreakthroughLevel(session: SessionData): number {
    ...
    if (session.history.some(h => h.antifragileProperties && h.antifragileProperties.length > 0)) score += 1;
