...
export function discoverTechniques(
  ...
  let integrationSuggestions = buildIntegrationSuggestions(
...
function categorizeProblem(problem: string, context?: string): string {
...
function adjustForPreferredOutcome(
  recommendations: Array<{ technique: LateralTechnique; reasoning: string; effectiveness: number }>,
  outcome: string
): void {
  switch (outcome) {
    case 'innovative':
...
function hasTimeConstraint(problem: string, constraints?: string[]): boolean {
...
function needsCollaboration(problem: string, context?: string): boolean {
 ...
 */
function generateHistoricalRelevance(
  category: string,
  preferredOutcome: string | undefined
): string {
 ...
 */
function generateSearchableFactors(
  problem: string,
  context: string | undefined,
  category: string,
  constraints: string[] | undefined
