 ...
 * caught in vetting).
 */
function combineBiasMaps(
  cruxBias: Partial<Record<LateralTechnique, number>> | undefined,
  personaBias: Partial<Record<LateralTechnique, number>> | undefined
): Partial<Record<LateralTechnique, number>> | undefined {
  if (!cruxBias) return personaBias;
  if (!personaBias) return cruxBias;
  const combined: Partial<Record<LateralTechnique, number>> = { ...personaBias };
  for (const [technique, value] of Object.entries(cruxBias) as Array<[LateralTechnique, number]>) {
...
export class TechniqueRecommender {
   ...
   */
  private readonly PERSONA_BASE_WEIGHT = 0.7;
  private readonly PERSONA_BIAS_WEIGHT = 0.3;
  ...
  recommendTechniques(
    ...
    cruxBias?: Partial<Record<LateralTechnique, number>>
    ...
    switch (problemCategory) {
      ...
      case 'adversarial':
        recommendations.push({
          ...
          reasoning:
            'Build the opposing case until its holders would sign it, then attack the plan through it',
          effectiveness: TECHNIQUE_FIT.DEFINING,
        });
        recommendations.push({
          technique: 'competing_hypotheses',
          reasoning:
            'Generate the alternatives the plan assumes away, and price the deception case',
          effectiveness: TECHNIQUE_FIT.PRIMARY,
        });
        recommendations.push({
          technique: 'cognitive_bias_audit',
          reasoning:
            'Name the tendencies that made the plan feel safe, and that will resist the findings',
          effectiveness: TECHNIQUE_FIT.STRONG,
        });
        break;
    ...
    if (cruxBias) {
      ...
      for (const [technique, fit] of Object.entries(cruxBias) as Array<
        [LateralTechnique, number]
      >) {
    ...
    const maxRecommendations = parseInt(
      process.env.MAX_TECHNIQUE_RECOMMENDATIONS || String(limits.max)
    );
