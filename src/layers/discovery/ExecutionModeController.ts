...
export class ExecutionModeController {
  constructor(
    private detector: ParallelismDetector,
    private validator: ParallelismValidator
  ) {}
  ...
  determineExecutionMode(
    ...
    if (detection.executionMode === 'parallel' || input.executionMode === 'auto') {
      ...
      const validation = this.validator.validateParallelRequest(
  ...
  private shouldAutoSelectParallel(
    ...
    const hasTimePressure = input.constraints?.some(
      c =>
   ...
   */
  private determineConvergence(input: DiscoverTechniquesInput): ConvergenceOptions {
    const convergenceDetection = this.detector.detectConvergenceIntent(
      input.problem,
      input.context
    );
