...
export class SynthesisPromptGenerator {
  generateSynthesisPrompts(
    structuredResults: StructuredResults,
    problem: string,
    strategy: PromptStrategy
  ): SynthesisPrompt[] {
    const prompts: SynthesisPrompt[] = [];
  ...
  private createCoreSynthesisPrompt(results: StructuredResults, problem: string): SynthesisPrompt {
...
Consider using sequential thinking or other tools if deeper analysis would be beneficial.`,
  ...
  private createComprehensivePrompts(results: StructuredResults): SynthesisPrompt[] {
    const techniqueCount =
      results.type === 'hierarchical'
...
- How can we maintain flexibility while pursuing multiple approaches?`,
  ...
  private createFocusedPrompts(results: StructuredResults): SynthesisPrompt[] {
...
- Measurable outcomes`,
  ...
  private createActionOrientedPrompts(results: StructuredResults): SynthesisPrompt[] {
    const ideaCount =
      results.type === 'hierarchical'
...
- Overall success criteria`,
  ...
  private createRiskAwarePrompts(results: StructuredResults): SynthesisPrompt[] {
...
- What options preserve future flexibility?`,
  ...
  private createConflictResolutionPrompt(results: StructuredResults): SynthesisPrompt {
...
4. Suggest how to test assumptions before committing`,
  ...
  private createComplexityManagementPrompt(results: StructuredResults): SynthesisPrompt {
...
Consider using sequential thinking to break down the complexity systematically.`,
  ...
  private createInnovationRefinementPrompt(results: StructuredResults): SynthesisPrompt {
    const innovativeCount =
      results.type === 'hierarchical'
        ? results.techniques.filter(t =>
            t.keyInsights.some(i => i.toLowerCase().includes('innovat'))
          ).length
        : 0;
...
Focus on maintaining the innovative spirit while ensuring feasibility.`,
  ...
  private hasConflicts(results: StructuredResults): boolean {
  ...
  private hasHighComplexity(results: StructuredResults): boolean {
    const ideaCount = this.getTotalIdeas(results);
    const techniqueCount = this.getTechniqueCount(results);
    return ideaCount > 30 || techniqueCount > 4;
  }

  private hasInnovativeIdeas(results: StructuredResults): boolean {
    if (results.type === 'flat') {
      return results.allIdeas.some(
        idea =>
          idea.tags?.includes('innovation') || idea.content.toLowerCase().includes('innovative')
      );
    }
  ...
  private getTechniqueCount(results: StructuredResults): number {
    switch (results.type) {
      case 'hierarchical':
  ...
  private getTotalIdeas(results: StructuredResults): number {
    switch (results.type) {
      case 'hierarchical':
  ...
  private getTechniqueNames(results: StructuredResults): string[] {
    switch (results.type) {
      case 'hierarchical':
  ...
  private suggestSynthesisApproach(results: StructuredResults): string {
  ...
  private extractTopThemes(results: StructuredResults): string[] {
    if (results.type === 'hierarchical') {
  ...
  private extractAllRisks(results: StructuredResults): string[] {
    switch (results.type) {
      case 'hierarchical':
  ...
  private identifyConflicts(results: StructuredResults): Array<{ description: string }> {
    if (results.type === 'hierarchical') {
  ...
  private assessComplexity(results: StructuredResults): 'low' | 'medium' | 'high' {
    const ideaCount = this.getTotalIdeas(results);
    const techniqueCount = this.getTechniqueCount(results);

    if (ideaCount > 50 || techniqueCount > 5) return 'high';
    if (ideaCount > 20 || techniqueCount > 3) return 'medium';
    return 'low';
  }
}
