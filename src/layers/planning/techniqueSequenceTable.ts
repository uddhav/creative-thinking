...
export const SEQUENCE_TABLE: readonly SequenceTableRow[] = [
  // No row survives the evidence bar today; see the header.
];

const ORDER_SENSITIVE: ReadonlySet<SequenceRelation> = new Set(['SEQUENCE_STRONGLY', 'SEQUENCE']);
 ...
 */
export function lookupSequenceRow(
  first: LateralTechnique,
  second: LateralTechnique
): SequenceTableRow | undefined {
  for (const row of SEQUENCE_TABLE) {
    const [a, b] = row.pair;
    if (a === first && b === second) return row;
    if (!ORDER_SENSITIVE.has(row.relation) && a === second && b === first) return row;
  }
  return undefined;
}
