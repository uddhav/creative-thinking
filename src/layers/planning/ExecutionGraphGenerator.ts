...
export class ExecutionGraphGenerator {
   ...
   */
  private static computeDepths(nodes: ExecutionGraphNode[]): {
    depth: Map<string, number>;
    deepestParent: Map<string, string | undefined>;
  } {
    const byId = new Map(nodes.map(node => [node.id, node]));
    const depth = new Map<string, number>();
    const deepestParent = new Map<string, string | undefined>();
    const onStack = new Set<string>();

    const settle = (start: ExecutionGraphNode): void => {
      ...
      while (stack.length > 0) {
        ...
        for (const dep of node.dependencies) {
          ...
          if (onStack.has(dependency.id)) continue;
   ...
   * same round.
   */
  private static roundsFrom(nodes: ExecutionGraphNode[], depth: Map<string, number>): string[][] {
  ...
  private static deepestChain(
    ...
    let top = nodes[0];
    for (const node of nodes) {
      if ((depth.get(node.id) ?? 0) > (depth.get(top.id) ?? 0)) top = node;
    }
    // Every parent is exactly one level shallower, so the walk terminates.
    const path: string[] = [];
    let current: string | undefined = top.id;
