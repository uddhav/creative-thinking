...
function irreversibleMatrixCells(input: ExecuteThinkingStepInput): string[] {
  ...
  for (const raw of cells.slice(0, MAX_CELLS)) {
    ...
    byCoordinate.set(
      where,
      `Caller-declared (nine_windows ${where}): ${content}${suffix}`.slice(0, MAX_CONTENT * 2)
    );
  }
...
function resolveTechniqueInstance(
  ...
  const advance = (local: number): boolean =>
    local === 1 && held.has(1) && cursor < instances.length - 1;
