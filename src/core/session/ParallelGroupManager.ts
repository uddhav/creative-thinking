...
import type { LateralTechnique, SessionData } from '../../types/index.js';
import type {
...
export class ParallelGroupManager {
  ...
  createParallelSessionGroup(
    ...
    const metadata: ParallelGroupMetadata = {
  ...
  private createParallelSession(
    ...
    const parallelMetadata: ParallelMetadata = {
      planId: plan.planId,
   ...
   */
  private handleGroupCompletion(group: ParallelSessionGroup): void {
    console.error(
      `[ParallelGroupManager] Group ${group.groupId} completed with ${group.completedSessions.size} sessions`
    );
  ...
  getParallelResults(
    ...
    for (const sessionId of group.sessionIds) {
      const session = sessions.get(sessionId);
      if (!session || session.isConvergenceSession) continue;
   ...
   */
  private extractSessionResult(sessionId: string, session: SessionData): ParallelExecutionResult {
    ...
    return {
      ...
      results: typeof results === 'object' ? (results as Record<string, unknown>) : {},
      metrics: {
        executionTime:
  ...
  getActiveGroups(): ParallelSessionGroup[] {
    return Array.from(this.parallelGroups.values()).filter(
      group => group.status === 'active' || group.status === 'converging'
    );
  }
   ...
   */
  private extractUniqueTechniques(plans: ParallelPlan[]): LateralTechnique[] {
    const techniques = new Set<LateralTechnique>();
    for (const plan of plans) {
   ...
   */
  private estimateGroupCompletion(plans: ParallelPlan[]): number {
  ...
  getStats(): {
    ...
    return {
      ...
      completedGroups,
      failedGroups,
      averageSessionsPerGroup:
        this.parallelGroups.size > 0 ? totalSessions / this.parallelGroups.size : 0,
