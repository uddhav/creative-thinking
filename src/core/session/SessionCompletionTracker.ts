...
export class SessionCompletionTracker {
  ...
  private countTechniqueCompletedSteps(
    ...
    const submissionsByStep = new Map<number, number>();
    const record = (localStep: number): void => {
    ...
    for (const entry of techniqueHistory) {
      ...
      if (plan.workflow.length === 1) {
        ...
        const entryIsLocal =
          entry.totalSteps === techniqueSteps && entry.totalSteps !== planTotal
            ? true
