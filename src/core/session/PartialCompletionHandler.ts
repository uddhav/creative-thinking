 ...
 */
type PartialCompletionStrategy =
...
export class PartialCompletionHandler {
  ...
  handlePartialCompletion(
    ...
    // Categorize sessions
    const categorized = this.categorizeSessions(
      allSessions,
      completedSessions,
      failedSessions,
      sessionManager
  ...
  private determineStrategy(
    ...
    categorized: CategorizedSessions,
  ...
  private proceedWithAvailable(
    ...
    const canConverge = this.canProceedWithConvergence(
      group,
  ...
  private fallbackConvergence(
    ...
    const fallbackPlan = {
      ...
      technique: 'convergence' as const,
  ...
  private getCompletedResults(
    ...
    for (const sessionId of completedSessionIds) {
      ...
      results.push({
        ...
        results: typeof resultData === 'object' ? (resultData as Record<string, unknown>) : {},
        metrics: {
          executionTime:
  ...
  private canProceedWithConvergence(
    ...
    const minThreshold =
      group.convergenceOptions?.convergencePlan?.metadata?.minSessionsRequired ||
      Math.ceil(group.sessionIds.length * 0.5);

    if (completedCount < minThreshold) return false;

    // Check if we have enough diversity
    const techniques = new Set(results.map(r => r.technique));
    const minTechniques =
      group.convergenceOptions?.convergencePlan?.metadata?.minTechniquesRequired || 2;
  ...
  private getMissingTechniques(
    ...
    const missingTechniques = new Set<string>();
   ...
   */
  private generatePartialRecommendations(results: ParallelExecutionResult[]): string[] {
    const recommendations: string[] = [];

    // Analyze available results
    const totalInsights = results.reduce((sum, r) => sum + r.insights.length, 0);
    const avgConfidence =
      results.reduce((sum, r) => sum + (r.metrics?.confidence || 0), 0) / results.length;
    ...
    recommendations.push(
      'Focus synthesis on areas with strongest consensus',
      'Acknowledge gaps from missing techniques in final output'
    );
