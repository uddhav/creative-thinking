...
export interface ParallelSessionGroup {
  ...
  startTime: number;
  completedSessions: Set<string>; // Using Set for O(1) lookups
...
export interface PersistedParallelState {
  ...
  parallelGroups: Array<{
    ...
    group: Omit<ParallelSessionGroup, 'completedSessions'> & {
      completedSessions: string[]; // Array for JSON serialization
    };
  }>;
  sharedContexts: Array<{
    groupId: string;
    context: Omit<SharedContext, 'sharedThemes'> & {
      sharedThemes: Array<[string, number]>; // Array for JSON serialization
    };
  }>;
  progressData: Array<{
    groupId: string;
    progress: Omit<GroupProgress, 'sessionProgress'> & {
      sessionProgress: Array<[string, SessionProgress]>; // Array for JSON serialization
    };
  }>;
}
