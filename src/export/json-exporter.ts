...
export class JSONExporter extends BaseExporter {
  ...
  private enhanceHistoryEntry(
    ...
    if (entry.hatColor) enhanced.hatColor = entry.hatColor;
    if (entry.scamperAction) enhanced.scamperAction = entry.scamperAction;
    if (entry.provocation) enhanced.provocation = entry.provocation;
    if (entry.randomStimulus) enhanced.randomStimulus = entry.randomStimulus;
    if (entry.successExample) enhanced.successExample = entry.successExample;
    if (entry.initialIdea) enhanced.initialIdea = entry.initialIdea;
  ...
  private categorizeScore(score: number, type: string): string {
    switch (type) {
      // outputCompleteness is a 0-1 fraction, not a 0-100 score.
      case 'completeness':
        if (score >= 0.8) return 'Highly Complete';
        if (score >= 0.6) return 'Complete';
        if (score >= 0.4) return 'Partially Complete';
        return 'Sparse';
