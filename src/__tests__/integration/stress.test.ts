...
describe('Stress Tests - Extreme Loads', () => {
  ...
  describe('Extreme Concurrent Load', () => {
    it('should handle 1000 concurrent discovery requests', async () => {
      ...
      console.log(`Initial memory: Heap ${formatMemory(memoryBefore.heapUsed)}, RSS ${formatMemory(memoryBefore.rss)}`);

      // Process in batches
      for (let batch = 0; batch < totalRequests / batchSize; batch++) {
        const batchPromises = Array.from({ length: batchSize }, (_, i) => {
          const index = batch * batchSize + i;
          const outcomes = ['innovative', 'systematic', 'risk-aware', 'collaborative', 'analytical'] as const;
        ...
        if ((batch + 1) % 2 === 0) {
          ...
          console.log(`Processed ${(batch + 1) * batchSize} requests. Memory: Heap ${formatMemory(currentMemory.heapUsed)}`);
        }
      }
      ...
      console.log(`- Final memory: Heap ${formatMemory(memoryAfter.heapUsed)}, RSS ${formatMemory(memoryAfter.rss)}`);
  ...
  describe('Extreme Sequential Load', () => {
    it('should handle 1000 sequential steps in a single session', async () => {
      ...
      for (let i = 0; i < 1000; i++) {
        ...
        if ((i + 1) % 100 === 0) {
          ...
          console.log(`Processed ${i + 1} steps. Memory: Heap ${formatMemory(currentMemory.heapUsed)}`);
        }
      }
      ...
      console.log(`- Memory increase: ${formatMemory(memoryAfter.heapUsed - memoryBefore.heapUsed)}`);
  ...
  describe('Extreme Session Creation', () => {
    it('should handle creating and managing 500 concurrent sessions', async () => {
      ...
      for (let batch = 0; batch < totalSessions / batchSize; batch++) {
        ...
        // Log progress
        const currentMemory = getMemoryUsageMB();
        console.log(`Created ${(batch + 1) * batchSize} sessions. Memory: Heap ${formatMemory(currentMemory.heapUsed)}`);
      }
      ...
      const verificationPromises = sessionData.map(async ({ planId, sessionId }, index) => {
        ...
        return server.executeThinkingStep({
          ...
          ...(technique === 'concept_extraction' ? { extractedConcepts: [`Concept ${index}`] } : {}),
      ...
      if (errors.length > 0) {
        console.log(`Note: ${errors.length} sessions were cleaned up (expected behavior under extreme load)`);
        const errorData = safeJsonParse<{ error: { code: string; message: string } }>(errors[0].content[0].text);
        if (errorData.error.code === 'SESSION_NOT_FOUND') {
          console.log('Sessions were cleaned up due to memory management - this is expected behavior');
        }
      }
      ...
      console.log(`- Memory increase: ${formatMemory(memoryAfter.heapUsed - memoryBefore.heapUsed)}`);
      console.log(`- Memory per session: ${((memoryAfter.heapUsed - memoryBefore.heapUsed) / totalSessions).toFixed(2)}MB`);
    ...
    it('should handle sessions with extensive risk and mitigation data', async () => {
      ...
      for (let step = 1; step <= 5; step++) {
        ...
        const result = await server.executeThinkingStep({
          ...
          antifragileProperties: Array.from({ length: 20 }, (_, i) => `Antifragile property ${i + 1}`),
