...
import type { SessionSummary, RiskAssessment, SamplingError } from '../types.js';
...
export class SmartSummaryGenerator {
   ...
   */
  async generateSummary(session: SessionData): Promise<SessionSummary> {
    try {
      ...
      return this.handleError(session, error as SamplingError);
    }
  }
...
Focus on actionability and value extraction rather than just recapping what happened.`;
   ...
   */
  private buildUserPrompt(session: SessionData): string {
...
Keep the summary actionable and focused on value.`;
   ...
   */
  private formatSessionHistory(session: SessionData): string {
   ...
   */
  private parseSummary(aiResponse: string, session: SessionData): SessionSummary {
    ...
    try {
      ...
      if (sections['NEXT STEPS'] || sections['RECOMMENDED NEXT STEPS']) {
        summary.nextSteps = this.extractBulletPoints(
          sections['NEXT STEPS'] || sections['RECOMMENDED NEXT STEPS']
        ).slice(0, 5);
      }
   ...
   */
  private extractIdeas(text: string, techniques: string[]): SessionSummary['bestIdeas'] {
    const ideas: SessionSummary['bestIdeas'] = [];
    const lines = text.split('\n');
   ...
   */
  private extractRisks(text: string): RiskAssessment {
   ...
   * Extract action items
   */
  private extractActionItems(text: string): SessionSummary['actionItems'] {
    const items: NonNullable<SessionSummary['actionItems']> = [];
    const lines = text.split('\n');
   ...
   */
  private fallbackSummary(session: SessionData): SessionSummary {
    const techniques = session.techniques.join(', ');
    const stepCount = session.steps.length;
   ...
   */
  private handleError(session: SessionData, error: SamplingError): SessionSummary {
