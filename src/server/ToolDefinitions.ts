  ...
  inputSchema: {
    ...
    properties: {
      ...
      persona: {
        ...
        maxLength: 200,
        description:
          'Thinking personality (e.g., "rory_sutherland", "rich_hickey", "custom:Security-minded Rust engineer")',
      },
      personas: {
        type: 'array',
  ...
  inputSchema: {
    ...
    properties: {
      ...
      currentStep: {
        type: 'number',
        description:
          'REQUIRED: Current step number (1-based). Prefer numbering within the current ' +
      ...
      totalSteps: {
        type: 'number',
        description:
          'REQUIRED: Total number of steps in the plan, matching how currentStep is counted. ' +
      ...
      pathImpact: {
        type: 'object',
        description:
          "SCAMPER's measurement of what the modification costs in future freedom — " +
      ...
      temporalLandscape: {
        type: 'object',
        description:
          'Step 1: the shape of the available time. Rejected loudly if it is not an ' +
      ...
      matrix: {
        type: 'object',
        description:
          'Step 3. Rejected unless hypotheses, evidence and ratings are all present. ' +
        ...
        properties: {
          ...
          ratings: {
            type: 'object',
            description:
              'Diagnosticity of each evidence-hypothesis pairing, keyed ' +
      ...
      probabilities: {
        type: 'object',
        description:
          'Step 6. Posterior probability keyed by hypothesis, e.g. { "H1": 0.6, "H2": 0.4 }. ' +
      ...
      validityScore: {
        ...
        maximum: 100,
        description:
          'Step 5: assessed validity as a percentage. Drives the validity band reported ' +
          'at the end.',
      },
      ...
      pronounRatios: {
        type: 'object',
        description:
          'Step 3. Keyed by ratio, not by pronoun — iWe is the one that is read, and an ' +
      ...
      coherenceScore: {
        ...
        maximum: 100,
        description:
          'Step 6: narrative coherence as a percentage, not a fraction. The bands sit at ' +
      ...
      vacantSpaces: {
        type: 'array',
        description:
          'Step 2: the ground nobody is standing on. Every entry needs all four keys ' +
      ...
      antiMimeticStrategy: {
        description:
          'Step 3: how this deliberately stops resembling the field. A plain string, or ' +
      ...
      roryMode: {
        type: 'boolean',
        description:
          'Draw the stimulus from the behavioural-economics catalogue instead of at random.',
      },
      // Anecdotal Signal
      anecdoteCount: {
        type: 'integer',
      ...
      signals: {
        type: 'array',
        description:
          'Step 2: the anecdotes that might be signal. Every entry needs all four keys ' +
        ...
        items: {
          ...
          properties: {
            ...
            adoptionLevel: {
              ...
              maximum: 100,
              description:
                'Percentage of the market, not a fraction. Above 25 is reported as ' +
      ...
      interventions: {
        type: 'array',
        description:
          'Step 2: the changes to the context, not to the message. Every entry needs all ' +
      ...
      perceptionGaps: {
        type: 'array',
        description:
          'Step 1: where what is true and what is perceived come apart. Every entry needs ' +
      ...
      perceptionROI: {
        ...
        exclusiveMinimum: 0,
        description:
          'Step 5: return as a multiple of what the equivalent spend on the product itself ' +
      ...
      patternRecognition: {
        ...
        items: { type: 'string' },
        description:
          'Step 1: successful patterns recognised across techniques. Required; `patterns` is accepted instead.',
      },
      patterns: {
        type: 'array',
        items: { type: 'string' },
      ...
      learningHistory: {
        ...
        items: { type: 'string' },
        description:
          'Step 2: accumulated learnings and their contexts. Required; `accumulatedLearning` is accepted instead.',
      },
      accumulatedLearning: {
        type: 'array',
        items: { type: 'string' },
      ...
      strategyAdaptations: {
        ...
        items: { type: 'string' },
        description:
          'Step 3: how technique selection adapts. Required; `strategyEvolution` is accepted instead.',
      },
      metaSynthesis: {
        type: 'string',
        description:
          'Step 4: Meta-level synthesis of learning patterns. Required; ' +
      ...
      verbosity: {
        ...
        enum: ['minimal', 'full'],
        description:
          "Response size control. 'minimal' keeps the step acknowledgment (ids, counters, progress), steering (nextStepGuidance; under completionMetadata the per-technique techniqueStatuses with their skippedSteps, plus skippedTechniques and completionWarnings when any exist), and every verdict the server reached about THIS step: flexibility, ergodicity metrics, early warnings, escape recommendations, reflexivity warnings, option generation, the ruin verdict, and on scamper steps pathImpact (the server's reversibility judgment, with its per-step reversible flag) — plus newInsights (only this step's additions) and fieldsRecorded (the names of the technique fields the server read). It drops two kinds of field: echoes of your own input (problem, output, technique field values), and cumulative re-sends of what earlier steps already delivered (the full insights list, and modificationHistory — server-built, not an echo, but a re-send of every prior step's pathImpact). The final step's completion summary is always full; memory decoration (the suggestive outputs) is off under minimal on every step. Blocked and error responses have their own compact shapes and are unaffected by this setting. Default: 'minimal' since 3.0.0; 'full' (or RESPONSE_VERBOSITY=full) restores the pre-3.0 shape.",
      },
      stepReversibility: {
        type: 'object',
        description:
      ...
      deleteOptions: {
        type: 'object',
        description:
          'For sessionOperation: delete. Requires a persistence adapter, and confirm: true, ' +
