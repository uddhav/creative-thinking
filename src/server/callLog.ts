...
function append(line: unknown): void {
 ...
 */
export function recordCall(name: string, args: unknown): void {
 ...
 */
export function recordResult(name: string, response: ToolResponse): void {
  if (!process.env.CT_CALL_LOG) return;
  ...
  try {
    ...
    if (text !== undefined) {
      ...
      if (typeof parsed.sessionId === 'string') summary.sessionId = parsed.sessionId;
      if (typeof parsed.planId === 'string') summary.planId = parsed.planId;
      if (typeof parsed.currentStep === 'number') summary.currentStep = parsed.currentStep;
      if (typeof parsed.technique === 'string') summary.technique = parsed.technique;
      ...
      if (Array.isArray(findings) && findings.length > 0) {
        summary.advisoryFindings = findings.map(f => {
          const finding = f as { gate?: unknown; severity?: unknown };
