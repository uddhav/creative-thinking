...
export function persistPlan(server: LateralThinkingServer, planId: string | undefined): void {
  ...
  try {
    ...
    process.stderr.write(
      `[socketes] Warning: failed to persist plan ${planId}: ${(err as Error).message}\n`
    );
  }
}
...
export function hydratePlan(server: LateralThinkingServer, planId: string): void {
  ...
  if (sessionManager.getPlan(planId)) return;
  ...
  try {
    ...
    process.stderr.write(
      `[socketes] Warning: failed to load plan ${planId}: ${(err as Error).message}\n`
    );
  }
}
