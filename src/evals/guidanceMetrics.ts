...
export function analyzeTechnique(
  ...
  return {
    ...
    totalSteps,
    stepsReferencingProblem,
    problemInterpolationRate:
      totalSteps === 0 ? 0 : roundTo(stepsReferencingProblem / totalSteps, 2),
 ...
 */
export function analyzeAllTechniques(registry: TechniqueRegistry): TechniqueGuidanceMetrics[] {
  return registry
    .getAllTechniques()
 ...
 */
export function summarize(metrics: TechniqueGuidanceMetrics[]): GuidanceMetricsSummary {
  ...
  return {
    ...
    totalSteps,
    stepsReferencingProblem,
    overallProblemInterpolationRate:
      totalSteps === 0 ? 0 : roundTo(stepsReferencingProblem / totalSteps, 2),
    meanLengthSpreadRatio:
      means.length === 0 ? 0 : roundTo(Math.max(...means) / Math.min(...means), 1),
 ...
 */
export function formatReport(metrics: TechniqueGuidanceMetrics[]): string {
  ...
  const rows = metrics
    ...
    .map(
      m =>
        `  ${m.technique.padEnd(26)} ${String(m.stepsReferencingProblem).padStart(2)}/${String(
          m.totalSteps
        ).padEnd(
          2
        )} steps  (${(m.problemInterpolationRate * 100).toFixed(0).padStart(3)}%)  mean ${String(
          m.meanGuidanceLength
        ).padStart(5)} chars`
    )
    .join('\n');
  ...
  return [
    ...
    `  steps referencing their problem: ${summary.stepsReferencingProblem}/${summary.totalSteps} (${(
      summary.overallProblemInterpolationRate * 100
    ).toFixed(0)}%)`,
